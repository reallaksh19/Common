...
ROLE_KIND = {role: cfg["intelligence_kind"] for role, cfg in POLICY["roles"].items()}
ROLE_CLAIM_TYPES = {role: set(cfg["claim_types"]) for role, cfg in POLICY["roles"].items()}
...
def _validate_bound_inputs(manifest: dict, run: dict, bundle_ref: str) -> dict:
    ...
    if run["current_state"] not in {"ROUTED", "FIRST_CORE_COMPLETE", "SECOND_CORE_COMPLETE", "CROSS_VALIDATED"}:
...
def _seal_work_order(order: dict) -> dict:
    ...
    out["work_order_id"] = "MATH-WO-" + digest({k: v for k, v in out.items() if k not in {"work_order_id", "work_order_digest"}})[:16]
...
def validate_work_order(order: dict, manifest: dict, run: dict) -> None:
    ...
    if order["ground_truth_ref"] != manifest["manifest_id"] or order["ground_truth_digest"] != manifest["manifest_digest"]:
    ...
    if set(order["allowed_ground_truth_evidence_refs"]) != evidence_ids:
    ...
    if order["phase"] in {"SECOND_ROLE_INDEPENDENT", "SECOND_ROLE_VALIDATION"} and order["role"] != OPPOSITE[bundle["first_role"]]:
...
def build_first_work_order(manifest: dict, run: dict, bundle_ref: str, agent_instance_id: str) -> dict:
    bundle = _validate_bound_inputs(manifest, run, bundle_ref)
    ...
    return _seal_work_order({
        ...
        "intelligence_kind": ROLE_KIND[bundle["first_role"]],
...
def validate_specialist_package(package: dict) -> None:
    ...
    for claim in package["claims"]:
        unknown_dependencies = set(claim["dependency_claim_refs"]) - claim_ids
        if unknown_dependencies:
            fail("SPECIALIST_CLAIM_DEPENDENCY_UNKNOWN", sorted(unknown_dependencies)[0])
...
def build_second_independent_work_order(
    ...
    if first_package["run_ref"] != run["run_id"] or first_package["bundle_ref"] != bundle_ref:
    ...
    second_role = OPPOSITE[bundle["first_role"]]
    return _seal_work_order({
        "work_order_id": "",
        "schema_version": "1.0.0",
        "subject": "MATHEMATICS",
        ...
        "bundle_ref": bundle_ref,
        "role": second_role,
        "intelligence_kind": ROLE_KIND[second_role],
...
def build_validation_work_order(
    ...
    if first_package["role"] != bundle["first_role"] or independent_package["role"] != OPPOSITE[bundle["first_role"]]:
        fail("VALIDATION_PACKAGE_ROLE_MISMATCH")
    ...
    if independent_package["phase"] != "SECOND_ROLE_INDEPENDENT" or not independent_package["sealed_before_upstream_exposure"]:
    ...
    return _seal_work_order({
        ...
        "allowed_upstream_package_refs": sorted([first_package["package_id"], independent_package["package_id"]]),
        "withheld_upstream_package_refs": [],
...
def materialize_context(
    ...
    *,
    ...
    _validate_bound_inputs(manifest, run, work_order["bundle_ref"])
    ...
    context = {
        ...
        "context_mode": "GROUND_TRUTH_PLUS_UPSTREAM_VALIDATION" if work_order["phase"] == "SECOND_ROLE_VALIDATION" else "GROUND_TRUTH_ONLY",
    ...
    context["context_id"] = "MATH-CTX-" + digest({k: v for k, v in context.items() if k not in {"context_id", "context_digest"}})[:16]
    context["context_digest"] = digest(context, "context_digest")
    validate_schema(context, "math-agent-context-manifest.schema.json")
    if work_order["phase"] == "SECOND_ROLE_INDEPENDENT" and context["upstream_packages"]:
        fail("INDEPENDENCE_FIREWALL_UPSTREAM_LEAK")
    return context


def _validate_context(context: dict, work_order: dict) -> None:
    validate_schema(context, "math-agent-context-manifest.schema.json")
    if context["context_digest"] != digest(context, "context_digest"):
        fail("AGENT_CONTEXT_DIGEST_MISMATCH")
    if context["work_order_ref"] != work_order["work_order_id"] or context["work_order_digest"] != work_order["work_order_digest"]:
        fail("AGENT_CONTEXT_WORK_ORDER_MISMATCH")
    if context["agent_instance_id"] != work_order["agent_instance_id"]:
        fail("AGENT_CONTEXT_INSTANCE_MISMATCH")
...
def seal_specialist_package(
    ...
    bundle = _validate_bound_inputs(manifest, run, work_order["bundle_ref"])
    validate_work_order(work_order, manifest, run)
    _validate_context(context, work_order)
    ...
    for raw in raw_claims:
        ...
        if claim_type not in ROLE_CLAIM_TYPES[work_order["role"]]:
            fail("SPECIALIST_CLAIM_TYPE_WRONG_ROLE", str(claim_type))
        subtopic_ref = raw.get("subtopic_ref")
        if subtopic_ref not in bundle["subtopic_refs"]:
            fail("SPECIALIST_CLAIM_SUBTOPIC_OUTSIDE_BUNDLE", str(subtopic_ref))
        evidence_refs = sorted(set(raw.get("evidence_refs") or []))
        if not evidence_refs:
            fail("SPECIALIST_CLAIM_EVIDENCE_REQUIRED", local_key)
        unknown = set(evidence_refs) - set(idx)
        if unknown:
        ...
        if all(idx[e]["availability"] == "ABSENT" for e in evidence_refs) and confidence != "UNKNOWN":
        ...
        claim_id = "MATH-CL-" + digest(base)[:16]
        local_to_id[local_key] = claim_id
        prepared.append((local_key, claim_id, raw, base, confidence))

    claims = []
    for local_key, claim_id, raw, base, confidence in prepared:
        ...
        try:
            deps = sorted(local_to_id[k] for k in dep_keys)
        ...
        claim = {
            ...
            "subtopic_ref": base["subtopic_ref"],
    ...
    package["package_id"] = "MATH-SP-" + digest({k: v for k, v in package.items() if k not in {"package_id", "package_digest"}})[:16]
...
def build_cross_validation(
    ...
    _validate_bound_inputs(manifest, run, first_package["bundle_ref"])
    validate_specialist_package(first_package)
    validate_specialist_package(independent_package)
    validate_work_order(validation_work_order, manifest, run)
    _validate_context(validation_context, validation_work_order)
    ...
    evidence = set(_evidence_index(manifest))
    records = []
    seen_first = set()
    for raw in authored.get("records") or []:
        ...
        if claim_ref in seen_first:
            fail("CROSS_VALIDATION_FIRST_CLAIM_DUPLICATE", claim_ref)
        seen_first.add(claim_ref)
        classification = raw.get("classification")
        if classification not in {"CONFIRMED", "REFINED", "UNSUPPORTED", "CONTRADICTED", "OUT_OF_SCOPE", "UNKNOWN"}:
            fail("CROSS_VALIDATION_CLASSIFICATION_UNKNOWN", str(classification))
        refs = sorted(set(raw.get("ground_truth_evidence_refs") or []))
        if set(refs) - evidence:
            fail("CROSS_VALIDATION_EVIDENCE_UNKNOWN", sorted(set(refs) - evidence)[0])
        ...
        records.append({"record_id": "MATH-XR-" + digest(rec_base)[:16], **rec_base})

    missing_first = set(first_claims) - seen_first
    if missing_first:
    ...
    for raw in authored.get("missing_findings") or []:
        ...
        missing_findings.append({"record_id": "MATH-XR-" + digest(rec_base)[:16], **rec_base})
    ...
    validation["validation_id"] = "MATH-XV-" + digest({k: v for k, v in validation.items() if k not in {"validation_id", "validation_digest"}})[:16]
...
def apply_cross_validation(run: dict, first_package: dict, independent_package: dict, validation: dict) -> dict:
    ...
    bundle = _bundle(run, validation["bundle_ref"])
    if first_package["package_id"] != validation["first_package_ref"] or independent_package["package_id"] != validation["independent_package_ref"]:
        fail("CROSS_VALIDATION_PACKAGE_BINDING_MISMATCH")

    out = copy.deepcopy(run)
    target = _bundle(out, bundle["bundle_id"])
    packages_by_role = {first_package["role"]: first_package, independent_package["role"]: independent_package}
    target["core1_execution_ref"] = packages_by_role["CORE1"]["package_id"]
    target["core2_execution_ref"] = packages_by_role["CORE2"]["package_id"]
    target["cross_validation_ref"] = validation["validation_id"]

    def first_done(b: dict) -> bool:
        return bool(b["core1_execution_ref"] if b["first_role"] == "CORE1" else b["core2_execution_ref"])

    conditions = {
        "FIRST_CORE_COMPLETE": bool(out["bundles"]) and all(first_done(b) for b in out["bundles"]),
        "SECOND_CORE_COMPLETE": bool(out["bundles"]) and all(b["core1_execution_ref"] and b["core2_execution_ref"] for b in out["bundles"]),
    ...
    for milestone in MILESTONES:
        if conditions[milestone] and milestone not in existing:
            out["current_state"] = milestone
            out["state_history"].append({
                "sequence": len(out["state_history"]),
                "state": milestone,
                "reason_code": "DUAL_INTELLIGENCE_" + milestone,
            })
...
def build_session(
    ...
    if session["first_agent_instance_id"] == session["second_agent_instance_id"]:
        fail("SELF_VALIDATION_INSTANCE_REUSE", session["first_agent_instance_id"])
    session["session_id"] = "MATH-DIS-" + digest({k: v for k, v in session.items() if k not in {"session_id", "session_digest"}})[:16]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Mathematics dual-intelligence runtime with enforced independent re-grounding.")
    sub = ap.add_subparsers(dest="command", required=True)

    p = sub.add_parser("first-order")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--bundle-ref", required=True); p.add_argument("--agent-instance-id", required=True); p.add_argument("--out", required=True)

    p = sub.add_parser("second-order")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--bundle-ref", required=True); p.add_argument("--first-package", required=True); p.add_argument("--agent-instance-id", required=True); p.add_argument("--out", required=True)

    p = sub.add_parser("context")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--work-order", required=True); p.add_argument("--package", action="append", default=[]); p.add_argument("--out", required=True)

    p = sub.add_parser("seal-package")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--work-order", required=True); p.add_argument("--context", required=True); p.add_argument("--draft", required=True); p.add_argument("--out", required=True)

    p = sub.add_parser("validation-order")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--bundle-ref", required=True); p.add_argument("--first-package", required=True); p.add_argument("--independent-package", required=True); p.add_argument("--out", required=True)

    p = sub.add_parser("cross-validate")
    p.add_argument("--ground-truth", required=True); p.add_argument("--run", required=True); p.add_argument("--first-work-order", required=True); p.add_argument("--first-context", required=True); p.add_argument("--first-package", required=True); p.add_argument("--second-work-order", required=True); p.add_argument("--second-context", required=True); p.add_argument("--independent-package", required=True); p.add_argument("--validation-work-order", required=True); p.add_argument("--validation-context", required=True); p.add_argument("--validation-input", required=True); p.add_argument("--out-validation", required=True); p.add_argument("--out-session", required=True); p.add_argument("--out-run", required=True)
    ...
    if args.command == "first-order":
        result = build_first_work_order(manifest, run, args.bundle_ref, args.agent_instance_id); _write(args.out, result)
    elif args.command == "second-order":
        result = build_second_independent_work_order(manifest, run, args.bundle_ref, load(args.first_package), args.agent_instance_id); _write(args.out, result)
    elif args.command == "context":
        result = materialize_context(manifest, run, load(args.work_order), packages=[load(x) for x in args.package]); _write(args.out, result)
    elif args.command == "seal-package":
        result = seal_specialist_package(manifest, run, load(args.work_order), load(args.context), load(args.draft)); _write(args.out, result)
    elif args.command == "validation-order":
        result = build_validation_work_order(manifest, run, args.bundle_ref, load(args.first_package), load(args.independent_package)); _write(args.out, result)
    else:
        first_order = load(args.first_work_order); first_context = load(args.first_context); first_package = load(args.first_package)
        second_order = load(args.second_work_order); second_context = load(args.second_context); independent_package = load(args.independent_package)
        validation_order = load(args.validation_work_order); validation_context = load(args.validation_context)
        validation = build_cross_validation(manifest, run, first_package, independent_package, validation_order, validation_context, load(args.validation_input))
        updated = apply_cross_validation(run, first_package, independent_package, validation)
        session = build_session(updated, first_order, first_context, first_package, second_order, second_context, independent_package, validation_order, validation_context, validation)
    ...
    print(json.dumps({"status": "PASS", "command": args.command, "result": result if isinstance(result, dict) else None}, indent=2))
