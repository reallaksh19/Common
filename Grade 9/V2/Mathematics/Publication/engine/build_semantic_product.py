...
def build(design_result):
    ...
    contents=[
      ...
      item('C02','PREDICT_BOX',['E01'],'Predict before you move',['Equation: 3x + 5 = 23','Proposed move: 3x = 18','Before calculating, mark LEGAL or NOT LEGAL and name the operation that would justify the move.'],'Predict whether a concrete proposed algebraic move preserves the same solution set.',1,{'concrete_proposed_move':True}),
      ...
      item('C04','MINIMAL_CONTRAST_PANEL',['E04'],'One difference: legal or illegal?',['Start: 4x + 7 = 19','Move A: subtract 7 from both sides -> 4x = 12','Move B: delete +7 from the left only -> 4x = 19','Predict which move preserves the same solution set. Explain the one difference before resolving.'],'Discriminate two explicit nearly identical transformations using one focal distinction.',2,{'prediction_before_resolution':e04['prediction_before_resolution'],'focal_distinction':e04['focal_distinction'],'concrete_pair':True}),
      ...
      item('C11','MINIMAL_CONTRAST_PANEL',['B03'],'Where did the middle term go?',['Same start: (x - 3)^2 = (x - 3)(x - 3)','Expansion A: x^2 - 6x + 9','Expansion B: x^2 + 9','Predict which expansion preserves every product. Explain where the middle term comes from before resolving.'],'Discriminate correct expansion from explicit structure loss.',2,{'prediction_before_resolution':True,'focal_distinction':b03['focal_distinction'],'concrete_pair':True}),
      ...
      item('C15','REPRESENTATION_PANEL',['S04'],'Separate strength check: representation bridge',['This is not part of the slope diagnosis. It is a separate preserved-strength check.',f"Source: {sm['S04']['payload']['source']}",f"Destination: {sm['S04']['payload']['destination']}",f"Job: {sm['S04']['payload']['translation_job']}",f"Preserve: {sm['S04']['payload']['invariant']}"],'Translate between representations while preserving the represented quantity.',1,{'separate_from_probe':True}),
      ...
      item('C17','COMPLETION_EVIDENCE_PANEL',['E12','V02'],'Finish by proving independence',['Independent completion task: solve 7 - 3(2x - 1) = 2x + 12.','Show one legal mathematical transformation per line.','When you believe you are finished, stop.'],'Provide observable independent completion evidence, including whether verification is initiated without a prompt.',0,{'prompted':False,'concrete_task':True})
    ]
    return {'semantic_product_id':'MATH-SEMANTIC-PRODUCT-SYN-002','schema_version':'1.0.0','subject':'MATHEMATICS','learning_design_id':plan['learning_design_id'],'learning_design_digest':plan['design_digest'],'fixture_class':'PUBLIC_SYNTHETIC','authority':'LEARNER_SEMANTIC_PRODUCT','content_items':contents}
