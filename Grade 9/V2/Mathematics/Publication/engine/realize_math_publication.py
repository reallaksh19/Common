...
WORKSPACE_LINES={'GUIDED_WORKSPACE':3,'FADED_WORKSPACE':3,'INDEPENDENT_WORKSPACE':4,'DIAGNOSTIC_PROBE_PANEL':4,'COMPLETION_EVIDENCE_PANEL':5}
EXTRA_HEIGHT={'GUIDED_WORKSPACE':66,'FADED_WORKSPACE':60,'INDEPENDENT_WORKSPACE':78,'DIAGNOSTIC_PROBE_PANEL':72,'COMPLETION_EVIDENCE_PANEL':84,'PREDICT_BOX':16,'MINIMAL_CONTRAST_PANEL':18}
...
def wrap(text,font='Helvetica',size=10.5,max_width=490):
...
def build_structure(product,policy):
    ...
    return {'structure_id':'MATH-PUB-STRUCTURE-SYN-002','schema_version':'1.0.0','semantic_product_ref':product['semantic_product_id'],'page_intents':intents}

def panel_height(item):
    body_lines=sum(len(wrap(x)) for x in item['body'])
    return max(92,58+body_lines*14+EXTRA_HEIGHT.get(item['primitive'],0))
...
def draw_panel(c,item,page_intent_id,y):
    ...
    c.setLineWidth(0.9); c.rect(x,y0,w,h,stroke=1,fill=0)
    c.setFont('Helvetica-Bold',12.5); c.drawString(x+12,y-20,item['title'])
    ty=y-40; c.setFont('Helvetica',10.5)
    for para in item['body']:
        for line in wrap(para,size=10.5,max_width=w-24): c.drawString(x+12,ty,line); ty-=14
        ty-=4
    for _ in range(WORKSPACE_LINES.get(item['primitive'],0)):
        c.line(x+14,ty-3,x+w-14,ty-3); ty-=16
    c.setFont('Helvetica-Oblique',6.5); c.drawRightString(x+w-8,y0+7,f"{item['content_id']} | {item['primitive']}")
    return {'content_ref':item['content_id'],'page_intent_id':page_intent_id,'primitive':item['primitive'],'x0':round(x,2),'y0':round(y0,2),'x1':round(x+w,2),'y1':round(y,2),'fragment_kind':'START'}, y0-13

def render(product,structure,pdf_path):
    items={i['content_id']:i for i in product['content_items']}; placements=[]
    c=canvas.Canvas(str(pdf_path),pagesize=A4,invariant=1,pageCompression=0); c.setTitle('MATH-V2-06 revised Mathematics learner candidate')
    for intent in structure['page_intents']:
        c.setFont('Helvetica-Bold',17); c.drawString(MARGIN,PAGE_H-38,intent['title'])
        c.setFont('Helvetica',7.5); c.drawRightString(PAGE_W-MARGIN,PAGE_H-35,f"MATH-V2-06 | page {intent['page_number']}")
        y=PAGE_H-60
...
def realize(product,target,policy,out):
    ...
    page_map={'page_map_id':'MATH-PHYSICAL-PAGE-MAP-SYN-002','schema_version':'1.0.0','artifact_path':'candidate.pdf','artifact_sha256':pdf_sha,'page_count':len(structure['page_intents']),'actual_placement_evidence':True,'content_placements':placements}
    ...
    audit={'audit_id':'MATH-PUB-AUDIT-SYN-002','schema_version':'1.0.0','artifact_sha256':pdf_sha,'checks':checks,'publication_engineering':'PASS'}
    ...
    manifest={'manifest_id':'MATH-PUB-MANIFEST-SYN-002','schema_version':'1.0.0',**desc,'package_digest':sha_obj(desc),'quality_states':quality_states()}
