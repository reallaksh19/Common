...
def render_pdf(plan: dict, path: Path) -> None:
    ...
    story = [
        Paragraph(plan["title"], styles["C1BTitle"]),
        Paragraph("Open-ended self-guided consolidation", styles["C1BBody"]),
        Paragraph("Try first. Use only as much help as you need. Check after you have reconstructed the idea.", styles["C1BBody"]),
        Spacer(1, 8),
    ]
    for block in plan["blocks"]:
        ...
        if frame:
            story.append(Paragraph("<b>TRY / PREDICT FIRST</b>", styles["C1BBody"]))
            story.append(Paragraph(frame["open_question"], styles["C1BBody"]))
            story.append(Paragraph(f"<b>Focus:</b> {frame['reconstruction_focus']}", styles["C1BHelp"]))
            for label in frame["help_sequence"][:-1]:
                story.append(Paragraph(f"<b>{label.replace('_',' ').title()}:</b> {frame['help'][label]}", styles["C1BHelp"]))
        elif block.get("learner_text"):
            story.append(Paragraph(block["learner_text"], styles["C1BBody"]))
        if block.get("math_lines"):
            story.append(Paragraph("<b>Scaffold / worked resolution</b>", styles["C1BBody"]))
