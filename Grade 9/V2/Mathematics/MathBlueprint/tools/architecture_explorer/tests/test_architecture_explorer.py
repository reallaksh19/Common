#!/usr/bin/env python3
"""Tests for Blueprint Architecture Explorer."""
...
class ArchitectureExplorerTests(unittest.TestCase):
    ...
    def test_derived_relation_cannot_be_mislabeled_explicit(self):
        m = scan_components(BLUEPRINT_ROOT)
        # Any relation extracted via regex string matching (e.g. REFERENCES from markdown or schema mentions in text)
        # must be DERIVED, not EXPLICIT.
    ...
    def test_topic_specific_mathematical_ids_are_not_present_in_explorer_code(self):
        source = (TOOL_DIR / "generate_architecture_manifest.py").read_text(encoding="utf-8")
        # Ensure no topic-specific strings exist in the generator
        forbidden = ["quadratic", "euclid", "theory of equations", "vieta"]
