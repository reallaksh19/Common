...
def validate_gate_schema(registry: dict) -> None:
    ...
    try:
        ...
        raise MathematicsEngineeringGateValidationError("MATH_GATE_SCHEMA_VIOLATION", str(e), {"path": list(e.path)})
...
def validate_global_invariants(registry: dict) -> None:
    ...
    for gate in registry["subtopic_gates"]:
        ...
        if sub_id in seen_subtopics:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_DUPLICATE_ID", f"Duplicate subtopic ID: {sub_id}")
        seen_subtopics.add(sub_id)

        for c in gate["technical_core"]:
            cid = c["concept_id"]
            if cid in seen_concepts:
                raise MathematicsEngineeringGateValidationError("MATH_GATE_DUPLICATE_ID", f"Duplicate concept ID: {cid}")
            seen_concepts.add(cid)

        for eq in gate["mandatory_equations"]:
            eid = eq["equation_id"]
            if eid in seen_equations:
                raise MathematicsEngineeringGateValidationError("MATH_GATE_DUPLICATE_ID", f"Duplicate equation ID: {eid}")
            seen_equations.add(eid)

        for rep in gate["representations"]:
            rid = rep["representation_id"]
            if rid in seen_reps:
                raise MathematicsEngineeringGateValidationError("MATH_GATE_DUPLICATE_ID", f"Duplicate representation ID: {rid}")
            seen_reps.add(rid)

        for m in gate["misconceptions"]:
            mid = m["misconception_id"]
            if mid in seen_misconceptions:
                raise MathematicsEngineeringGateValidationError("MATH_GATE_DUPLICATE_ID", f"Duplicate misconception ID: {mid}")
...
def validate_subtopic_invariants(gate: dict) -> None:
    ...
    if not sub_id.startswith("MATH-"):
        raise MathematicsEngineeringGateValidationError("MATH_GATE_INVALID_SUBTOPIC_ID", f"Invalid subtopic ID format: {sub_id}")
    if gate.get("maturity") != "ENGINEERING":
        raise MathematicsEngineeringGateValidationError("MATH_GATE_MATURITY_OVERREACH", f"Maturity must be ENGINEERING, got {gate.get('maturity')}")
    ...
    # 3. Specific Mathematics Invariant Gates
    ...
    if sub_id == "MATH-NUM-RADICALS":
        if "CON-MATH-PRINCIPAL-SQUARE-ROOT-ABS" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-NUM-RADICALS requires principal square root non-negativity concept (CON-MATH-PRINCIPAL-SQUARE-ROOT-ABS)")
        if "EQ-MATH-RADICAL-IDENTITY" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-NUM-RADICALS requires radical identity equation (EQ-MATH-RADICAL-IDENTITY)")

    elif sub_id == "MATH-ALG-POLYNOMIALS":
        if "CON-MATH-FACTOR-THEOREM" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-ALG-POLYNOMIALS requires factor theorem concept (CON-MATH-FACTOR-THEOREM)")
        if "MISC-MATH-FRESHMANS-DREAM" not in misc_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MISCONCEPTION_TRAP", "MATH-ALG-POLYNOMIALS requires Freshman's dream trap (MISC-MATH-FRESHMANS-DREAM)")

    elif sub_id == "MATH-LIN-EQUATIONS":
        if "CON-MATH-LINEAR-SYSTEM-CONSISTENCY" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-LIN-EQUATIONS requires linear system consistency concept (CON-MATH-LINEAR-SYSTEM-CONSISTENCY)")
        if "EQ-MATH-RATIO-CONSISTENCY" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-LIN-EQUATIONS requires ratio consistency equation (EQ-MATH-RATIO-CONSISTENCY)")

    elif sub_id == "MATH-QUAD-EQUATIONS":
        if "MATH-ALG-POLYNOMIALS" not in prereq_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_UNRESOLVED_PREREQUISITE", "MATH-QUAD-EQUATIONS requires polynomial prerequisite (MATH-ALG-POLYNOMIALS)")
        if "CON-MATH-QUAD-NONZERO-LEAD" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-QUAD-EQUATIONS requires non-zero leading coefficient concept (CON-MATH-QUAD-NONZERO-LEAD)")
        if "EQ-MATH-QUAD-FORMULA" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-QUAD-EQUATIONS requires quadratic formula (EQ-MATH-QUAD-FORMULA)")

    elif sub_id == "MATH-GEO-COORDINATES":
        if "CON-MATH-VERTICAL-SLOPE-UNDEFINED" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-GEO-COORDINATES requires vertical slope undefined concept (CON-MATH-VERTICAL-SLOPE-UNDEFINED)")
        if "EQ-MATH-DISTANCE-FORMULA" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-GEO-COORDINATES requires distance formula (EQ-MATH-DISTANCE-FORMULA)")

    elif sub_id == "MATH-GEO-TRIANGLES":
        if "CON-MATH-CONGRUENCE-CRITERIA" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-GEO-TRIANGLES requires congruence criteria concept (CON-MATH-CONGRUENCE-CRITERIA)")
        if "MISC-MATH-SSA-CONGRUENCE-FALLACY" not in misc_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MISCONCEPTION_TRAP", "MATH-GEO-TRIANGLES requires SSA fallacy trap (MISC-MATH-SSA-CONGRUENCE-FALLACY)")
        if "REP-MATH-GEOMETRIC-TWO-COLUMN-PROOF" not in representation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_REPRESENTATION", "MATH-GEO-TRIANGLES requires two-column proof representation (REP-MATH-GEOMETRIC-TWO-COLUMN-PROOF)")

    elif sub_id == "MATH-TRIG-RATIOS":
        if "CON-MATH-TRIG-ACUTE-DOMAIN" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-TRIG-RATIOS requires acute domain concept (CON-MATH-TRIG-ACUTE-DOMAIN)")
        if "EQ-MATH-PYTHAGOREAN-TRIG-IDENTITY" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-TRIG-RATIOS requires Pythagorean identity (EQ-MATH-PYTHAGOREAN-TRIG-IDENTITY)")

    elif sub_id == "MATH-GEO-CIRCLES":
        if "CON-MATH-TANGENT-RADIUS-PERPENDICULAR" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-GEO-CIRCLES requires tangent-radius perpendicularity concept (CON-MATH-TANGENT-RADIUS-PERPENDICULAR)")
        if "EQ-MATH-CYCLIC-QUAD-SUPPLEMENTARY" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-GEO-CIRCLES requires cyclic quad supplementary equation (EQ-MATH-CYCLIC-QUAD-SUPPLEMENTARY)")

    elif sub_id == "MATH-MENS-SURFACES":
        if "CON-MATH-COMPOSITE-INTERFACE-EXCLUSION" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-MENS-SURFACES requires interface exclusion concept (CON-MATH-COMPOSITE-INTERFACE-EXCLUSION)")
        if "EQ-MATH-VOLUME-CONSERVATION" not in equation_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MANDATORY_EQUATION", "MATH-MENS-SURFACES requires volume conservation equation (EQ-MATH-VOLUME-CONSERVATION)")

    elif sub_id == "MATH-STAT-PROBABILITY":
        if "CON-MATH-PROBABILITY-BOUNDS" not in concept_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_REQUIRED_CONCEPT", "MATH-STAT-PROBABILITY requires probability bounds concept (CON-MATH-PROBABILITY-BOUNDS)")
        if "MISC-MATH-PROBABILITY-OUT-OF-BOUNDS" not in misc_ids:
            raise MathematicsEngineeringGateValidationError("MATH_GATE_MISSING_MISCONCEPTION_TRAP", "MATH-STAT-PROBABILITY requires probability bounds trap (MISC-MATH-PROBABILITY-OUT-OF-BOUNDS)")
    ...
    for d in dims:
        if not (0 <= dp.get(d, -1) <= 3):
            raise MathematicsEngineeringGateValidationError("MATH_GATE_INVALID_DIFFICULTY_PROFILE", f"Dimension {d} must be 0-3")
    if dp.get("maturity") != "ENGINEERING":
        raise MathematicsEngineeringGateValidationError("MATH_GATE_INVALID_DIFFICULTY_PROFILE", "Difficulty profile maturity must be ENGINEERING")
...
def run_falsification_battery():
    ...
    def expect_rejection(mutated: dict, expected_code: str):
        try:
            ...
            raise AssertionError(f"Expected MathematicsEngineeringGateValidationError [{expected_code}], got {type(err).__name__}: {err}")
        raise AssertionError(f"Expected validator rejection with code [{expected_code}], but validation passed!")

    # 1. MATH-FAIL-01: Radical square root producing negative without absolute value
    ...
    # 2. MATH-FAIL-02: Quadratic equation missing non-zero leading coefficient condition
    ...
    # 3. MATH-FAIL-03: Division by zero / vertical slope undefined condition omitted in coordinate geometry
    ...
    # 4. MATH-FAIL-04: Distance formula omitted in coordinate geometry
    ...
    # 5. MATH-FAIL-05: Freshman's dream (a+b)^2 = a^2+b^2 trap missing in polynomials
    ...
    # 6. MATH-FAIL-06: SSA accepted as congruence criterion in triangles
    ...
    # 7. MATH-FAIL-07: Probability outside [0, 1] accepted in statistics & probability
    ...
    # 8. MATH-FAIL-08: Quadratic equations stripped of polynomial prerequisite
    ...
    # 9. MATH-FAIL-09: Euclidean triangle proof missing two-column statement-reason representation
    ...
    # 10. MATH-FAIL-10: Problem family ID in linked_problem_family_ids not defined
    ...
    # 11. MATH-FAIL-11: Duplicate concept ID across distinct subtopics
    ...
    # 12. MATH-FAIL-12: Incomplete release checklist marked as ENGINEERING_GATE_READY
    ...
    print("Mathematics Technical Engineering Gate falsification battery: PASS (all 12 mutation falsifiers caught by production validator)")


if __name__ == "__main__":
    reg = load_json(ROOT / "policies" / "mathematics-technical-engineering-gates.v1.json")
