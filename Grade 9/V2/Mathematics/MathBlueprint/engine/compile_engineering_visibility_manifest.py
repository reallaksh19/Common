...
from validate_mathematics_engineering_binding import validate as validate_binding
...
NOTICE = (
    ...
    "it does not create mathematical authority and does not authorize publication."
)
...
def validate_visibility_manifest(manifest: dict) -> dict:
    validate_schema(manifest, "math-engineering-visibility-manifest.schema.json")
    if manifest["manifest_digest"] != digest(manifest, "manifest_digest"):
        fail("MATH_ENG_VISIBILITY_DIGEST_MISMATCH")
    expected_id = "MATH-ENG-VIS-" + digest({k: v for k, v in manifest.items() if k not in {"manifest_id", "manifest_digest"}})[:16]
    if manifest["manifest_id"] != expected_id:
        fail("MATH_ENG_VISIBILITY_ID_MISMATCH")
    if manifest["publication_authorization"] != "NOT_IMPLIED":
        fail("MATH_ENG_VISIBILITY_PUBLICATION_AUTHORITY_FORBIDDEN")
    if manifest["authorization_count"] != len(manifest["authorizations"]):
        fail("MATH_ENG_VISIBILITY_AUTHORIZATION_COUNT_DRIFT")
    if manifest["gate_count"] != len(manifest["gates"]):
        fail("MATH_ENG_VISIBILITY_GATE_COUNT_DRIFT")

    auth_ids = [row["authorization_id"] for row in manifest["authorizations"]]
    if len(auth_ids) != len(set(auth_ids)):
        fail("MATH_ENG_VISIBILITY_AUTHORIZATION_DUPLICATE")
    gate_ids = [row["gate_id"] for row in manifest["gates"]]
    if len(gate_ids) != len(set(gate_ids)):
        fail("MATH_ENG_VISIBILITY_GATE_DUPLICATE")
    auth_set = set(auth_ids)
    for gate in manifest["gates"]:
        unknown = sorted(set(gate["authorization_refs"]) - auth_set)
        if unknown:
            fail("MATH_ENG_VISIBILITY_GATE_AUTHORIZATION_UNKNOWN", ",".join(unknown))
        counts = gate["structure_counts"]
        if counts.get("representations") != len(gate["representations"]):
            fail("MATH_ENG_VISIBILITY_REPRESENTATION_COUNT_DRIFT", gate["gate_id"])
        if counts.get("misconceptions") != len(gate["misconceptions"]):
            fail("MATH_ENG_VISIBILITY_MISCONCEPTION_COUNT_DRIFT", gate["gate_id"])
        if counts.get("verification_obligations") != len(gate["verification_obligations"]):
            fail("MATH_ENG_VISIBILITY_VERIFICATION_COUNT_DRIFT", gate["gate_id"])
    return {
        "status": "PASS",
...
def compile_visibility_manifest(
    ...
    *,
    ...
    if release_gate.get("status") != "PASS":
        fail("MATH_ENG_VISIBILITY_RELEASE_GATE_REQUIRED")
    if admission.get("subject") != "MATHEMATICS":
        fail("MATH_ENG_VISIBILITY_SUBJECT")
    if not admission.get("authorizations"):
        fail("MATH_ENG_VISIBILITY_AUTHORIZATION_REQUIRED")

    registry = registry or load_engineering("policies/mathematics-technical-engineering-gates.v1.json")
    ...
    for auth in admission["authorizations"]:
        ...
        if passport["blueprint_technical_authorization"] != "ALLOWED":
            fail("MATH_ENG_VISIBILITY_TECHNICAL_AUTHORIZATION_REQUIRED", auth["authorization_id"])
        if passport["publication_authorization"] != "NOT_IMPLIED":
            fail("MATH_ENG_VISIBILITY_PASSPORT_PUBLICATION_BOUNDARY", auth["authorization_id"])
        ...
        for surface in passport["engineering_surface"]["gates"]:
            ...
            if gate is None:
                fail("MATH_ENG_VISIBILITY_GATE_MISSING_CURRENT_REGISTRY", gate_id)
    ...
    if len(registry_digests) != 1:
        fail("MATH_ENG_VISIBILITY_REGISTRY_CUSTODY_DISAGREEMENT")
    if len(validator_digests) != 1:
        fail("MATH_ENG_VISIBILITY_VALIDATOR_CUSTODY_DISAGREEMENT")
    ...
    out["manifest_id"] = "MATH-ENG-VIS-" + digest({k: v for k, v in out.items() if k not in {"manifest_id", "manifest_digest"}})[:16]
