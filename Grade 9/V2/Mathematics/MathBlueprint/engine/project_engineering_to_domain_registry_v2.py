...
def _canonical_asset_id(row: dict, asset_type: str) -> str:
    ...
    return f"REG-MATH-ENG-{asset_type}-{token}"
...
def _membership(
    ...
    payload["membership_id"] = "MATH-ENG-SCOPE-MEMBER-" + digest(payload)[:20].upper()
    payload["membership_digest"] = digest(payload)
    return payload


def _membership_material(row: dict) -> dict:
    return {k: v for k, v in row.items() if k != "membership_digest"}
...
def canonicalize_expanded_projection(
    ...
    receipt["projection_id"] = "MATH-ENG-DOMAIN-PROJ-V2-" + digest({
