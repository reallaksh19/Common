...
def _load_authorization_bundle(row: dict, injected: dict[str, dict] | None) -> tuple[dict, dict, dict]:
    ...
    if injected is not None:
        ...
        if bundle is None:
            raise MathematicsEngineeredDomainAdmissionError(
                "MATH_ENG_DOMAIN_AUTHORIZATION_INPUT_MISSING",
                auth_id,
            )
        try:
            ...
            raise MathematicsEngineeredDomainAdmissionError(
                "MATH_ENG_DOMAIN_AUTHORIZATION_INPUT_INCOMPLETE",
...
def validate(
    ...
    *,
    ...
    if len(auth_ids) != len(set(auth_ids)):
        raise MathematicsEngineeredDomainAdmissionError(
            "MATH_ENG_DOMAIN_DUPLICATE_AUTHORIZATION_ID",
    ...
    for row in auth_rows:
        ...
        for gate_id in result["direct_gate_ids"]:
            ...
            if previous is not None and previous != auth_id:
                raise MathematicsEngineeredDomainAdmissionError(
                    "MATH_ENG_DOMAIN_DIRECT_GATE_AUTHORIZATION_OVERLAP",
    ...
    if len(registry_digests) != 1:
        raise MathematicsEngineeredDomainAdmissionError(
            "MATH_ENG_DOMAIN_ENGINEERING_REGISTRY_DRIFT",
            ",".join(sorted(registry_digests)),
        )
    if len(validator_digests) != 1:
        raise MathematicsEngineeredDomainAdmissionError(
            "MATH_ENG_DOMAIN_VALIDATOR_CONTRACT_DRIFT",
    ...
    for row in rows:
        ...
        for gate_binding in row["gate_bindings"]:
            ...
            if gate_id in seen_gates:
                raise MathematicsEngineeredDomainAdmissionError(
                    "MATH_ENG_DOMAIN_DUPLICATE_GATE_MAPPING",
            ...
            if result is None:
                raise MathematicsEngineeredDomainAdmissionError(
                    "MATH_ENG_DOMAIN_AUTHORIZATION_REF_UNKNOWN",
            ...
            if gate_id not in set(result["direct_gate_ids"]):
                ...
                raise MathematicsEngineeredDomainAdmissionError(
                    "MATH_ENG_DOMAIN_GATE_NOT_DIRECTLY_AUTHORIZED",
    ...
    for auth_id, result in auth_results.items():
        ...
        if unused:
            raise MathematicsEngineeredDomainAdmissionError(
                "MATH_ENG_DOMAIN_UNUSED_DIRECT_GATE_AUTHORITY",
