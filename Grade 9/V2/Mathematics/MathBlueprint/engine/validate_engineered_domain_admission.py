...
def _schema_validate(doc: dict) -> None:
    ...
    if errors:
        e = errors[0]
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_ADMISSION_SCHEMA", f"{e.message}; path={list(e.path)}")
...
def validate(
    ...
    *,
    ...
    invariant_profile: dict | None = None,
) -> dict:
    _schema_validate(admission)
    if admission["subject"] != "MATHEMATICS":
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_SUBJECT_MISMATCH", "subject must be MATHEMATICS")
    ...
    if domain_registry.get("subject") != "MATHEMATICS":
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_SUBJECT_MISMATCH", "domain registry is not MATHEMATICS")

    try:
        domain_result = validate_domain_registry(domain_registry)
    except Exception as exc:
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_REGISTRY_INVALID", str(exc)) from exc
    ...
    try:
        ...
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_BINDING_INVALID", str(exc)) from exc
    ...
    if len(subtopic_ids) != len(set(subtopic_ids)):
        raise MathematicsEngineeredDomainAdmissionError("MATH_ENG_DOMAIN_DUPLICATE_SUBTOPIC_MAP", "duplicate subtopic mapping")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Validate Engineering-gated Mathematics Canonical Domain Registry admission")
