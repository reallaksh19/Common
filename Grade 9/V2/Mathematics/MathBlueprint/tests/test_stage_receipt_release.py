...
class StageReceiptReleaseTests(unittest.TestCase):
    ...
    def test_missing_semantic_asset_from_core1b_fails(self):
        reg=load(REGISTRY); receipts=make_receipts(reg); target="REG-MATH-EQ-VIETA_SUM"
        receipts[1]["coverage_claims"]=[x for x in receipts[1]["coverage_claims"] if target not in x["canonical_asset_refs"]]
        with self.assertRaisesRegex(ValueError,"ASSEMBLER_REQUIRED_ASSET_MISSING"): release(reg,receipts)

    def test_unbound_stage_cannot_release(self):
        reg=load(REGISTRY); receipts=make_receipts(reg); receipts[0]["registry_binding"]={"status":"UNBOUND","registry_ref":None,"matched_asset_refs":[]}; receipts[0]["release_state"]="UNBOUND_PRE_RELEASE"
        with self.assertRaisesRegex(ValueError,"ASSEMBLER_RECEIPT_REGISTRY_UNBOUND"): release(reg,receipts)

    def test_core1a_core1b_difficulty_drift_fails(self):
        reg=load(REGISTRY); receipts=make_receipts(reg); receipts[1]["difficulty_evidence"][0]["operational_badge"]="MEDIUM"
        with self.assertRaisesRegex(ValueError,"ASSEMBLER_CORE1A_CORE1B_DIFFICULTY_DRIFT"): release(reg,receipts)
    ...
    def test_learner_fit_over_ceiling_fails(self):
        ...
        with self.assertRaisesRegex(ValueError,"LEARNER_FIT_DEMAND_CEILING_EXCEEDED"): release(reg,receipts)
