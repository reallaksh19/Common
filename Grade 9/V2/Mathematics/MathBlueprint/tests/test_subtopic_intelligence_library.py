#!/usr/bin/env python3
"""
Unit Tests for Subtopic Intelligence Library (SIL) Intake Validator
===================================================================
...
class SubtopicIntelligenceLibraryTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_sil_validator_detects_missing_ttu_completion_keys(self) -> None:
        ...
        p["body"] = p["body"].replace("COMPLETION DERIVATION KEY", "SOLUTIONS OMITTED")
        result = validate_packet(p)
        self.assertEqual(result["status"], "FAIL")
