...
These primitives extend the existing Physics representation kit without adding topic
semantics. Defaults are schematic and intentionally contain no numerical quantities.
The stable interface mirrors the base renderer and returns measured vector-operation
evidence from the same TracingCanvas instrumentation.
"""
...
def _axes(c, x, y, w, h, *, origin_x=None, origin_y=None):
    ox = origin_x if origin_x is not None else x + w * 0.26
    oy = origin_y if origin_y is not None else y + h * 0.27
    xr = x + w - 24
    yt = y + h - 28
    c.setStrokeColor(Palette.TEXT_SECONDARY)
    c.setLineWidth(1.0)
    c.line(x + 20, oy, xr, oy)
    c.line(ox, y + 18, ox, yt)
    ...
    c.circle(ox, oy, 2.5, fill=1, stroke=0)
    c.setFont(FONT_BOLD, 6.4)
    c.drawString(xr + 3, oy - 2, "x")
    c.drawString(ox + 4, yt - 2, "y")
    c.setFont(FONT_NAME, 5.8)
    c.setFillColor(Palette.TEXT_MUTED)
    c.drawString(ox + 4, oy - 10, "origin")
    return ox, oy, xr, yt
...
def draw_cartesian_frame_2d(c, x, y, w, h, params):
    ...
    for frac in (0.45, 0.65, 0.85):
        gx = ox + (xr - ox) * frac
        c.line(gx, oy - 4, gx, oy + 4)
    for frac in (0.35, 0.6, 0.85):
        gy = oy + (yt - oy) * frac
        c.line(ox - 4, gy, ox + 4, gy)
...
def draw_vector_components_2d(c, x, y, w, h, params):
    ...
    c.drawCentredString((ox + px) / 2, oy - 12, params.get("x_component_label", "x component"))
    c.setFillColor(Palette.PHYSICS_MED)
    c.drawString(px + 5, (oy + py) / 2, params.get("y_component_label", "y component"))
...
def draw_state_sequence_2d(c, x, y, w, h, params):
    ...
    top = y + h - 44
    p0 = (left, base)
    p1 = (left + (right-left)*0.5, top)
    p2 = (right, base + 5)
    c.setStrokeColor(Palette.PHYSICS_DARK)
    c.setLineWidth(1.2)
    c.bezier(p0[0], p0[1], left+(right-left)*0.22, top, left+(right-left)*0.32, top, p1[0], p1[1])
    c.bezier(p1[0], p1[1], left+(right-left)*0.68, top, left+(right-left)*0.78, top*0.88, p2[0], p2[1])
    states = [p0, p1, p2]
    labels = params.get("state_labels") or ["t0", "event", "t2"]
    for i, (px, py) in enumerate(states):
        c.setFillColor(Palette.PHYSICS_BLUE if i != 1 else Palette.PHYSICS_MED)
        ...
        if vy:
            draw_arrow(c, px, py, px, py + vy, Palette.PHYSICS_MED, line_width=1.0)
        draw_arrow(c, px + 12, py + 24, px + 12, py + 8, Palette.DANGER, line_width=0.9)
        c.setFont(FONT_NAME, 5.9)
        c.setFillColor(Palette.TEXT_MUTED)
        c.drawCentredString(px, py - 12, str(labels[i])[:18])
    clock_y = y + 22
    c.setStrokeColor(Palette.TEXT_SECONDARY)
    c.setLineWidth(0.9)
    c.line(left, clock_y, right, clock_y)
    for px, _ in states:
        c.line(px, clock_y - 3, px, clock_y + 3)
    c.setFont(FONT_BOLD, 5.9)
    c.setFillColor(Palette.TEXT_SECONDARY)
    c.drawCentredString((left+right)/2, y + 10, "one shared clock")
...
def draw_path_anatomy_2d(c, x, y, w, h, params):
    ...
    c.setStrokeColor(Palette.PHYSICS_DARK)
    c.setLineWidth(1.2)
    c.bezier(left, base, left+40, apex_y, mid-35, apex_y, mid, apex_y)
    c.bezier(mid, apex_y, mid+35, apex_y, right-40, apex_y, right, base)
    ...
    c.setStrokeColor(Palette.BORDER_CARD)
    c.setDash(2, 2)
    c.line(left+42, base, left+42, base+34)
    c.setDash()
    c.setFont(FONT_NAME, 6.1)
    c.setFillColor(Palette.TEXT_SECONDARY)
    c.drawString(x + 22, y + 12, "Name the path events first; attach each vector to the event where it belongs.")
...
def draw_event_compare_2d(c, x, y, w, h, params):
    ...
    c.setLineWidth(1.2)
    c.bezier(left, base, left+85, y+h-92, right-65, y+h-92, right, base)
    for px, label in ((left, "common start"), (right, "common event")):
        c.setFillColor(Palette.PHYSICS_BLUE)
        c.circle(px, base, 4, fill=1, stroke=0)
        c.setFont(FONT_NAME, 5.8)
        c.setFillColor(Palette.TEXT_MUTED)
        c.drawCentredString(px, base - 13, label)
    c.setFont(FONT_BOLD, 6.0)
    c.setFillColor(Palette.PHYSICS_DARK)
    c.drawString(left + 28, y+h-42, "case A")
    c.setFillColor(Palette.PHYSICS_MED)
    c.drawRightString(right - 28, y+h-70, "case B")
    c.setFont(FONT_NAME, 6.1)
    c.setFillColor(Palette.TEXT_SECONDARY)
...
def draw_parametric_elimination_bridge_2d(c, x, y, w, h, params):
    ...
    for i, px in enumerate(xs):
        ...
        c.setFillColor(Palette.PHYSICS_DARK)
        c.drawCentredString(px + pw/2, py + ph - 14, str(titles[i])[:30])
        c.setStrokeColor(Palette.BORDER_LIGHT)
        c.line(px+8, py+ph-20, px+pw-8, py+ph-20)
    ...
    # Result panel contains only structural axes/curve, not a numeric equation.
    ox = xs[2] + 18
    oy = py + 20
    xr = xs[2] + pw - 12
    yt = py + ph - 28
    ...
    c.line(ox, oy, ox, yt)
    c.setStrokeColor(Palette.PHYSICS_DARK)
    c.bezier(ox, oy+4, ox+pw*0.2, yt, xr-pw*0.2, yt, xr, oy+4)
...
def draw_observer_line_of_sight_2d(c, x, y, w, h, params):
    ...
    c.setFillColor(colors.white)
    c.setStrokeColor(Palette.PHYSICS_DARK)
    c.circle(observer_x, ground + 10, 7, fill=1, stroke=1)
    c.line(observer_x, ground + 3, observer_x, ground - 8)
    c.line(observer_x, ground - 3, observer_x-7, ground-12)
    c.line(observer_x, ground - 3, observer_x+7, ground-12)
    # target and sight triangle
    c.setFillColor(Palette.PHYSICS_BLUE)
    c.circle(target_x, target_y, 4, fill=1, stroke=0)
    c.setStrokeColor(Palette.PHYSICS_DARK)
    c.setLineWidth(1.3)
    c.line(observer_x, ground+10, target_x, target_y)
    c.setStrokeColor(Palette.BORDER_CARD)
    c.setLineWidth(0.8)
    c.setDash(2, 2)
    c.line(observer_x, ground+10, target_x, ground+10)
    c.line(target_x, ground+10, target_x, target_y)
    c.setDash()
    c.setStrokeColor(Palette.PHYSICS_MED)
    c.arc(observer_x-4, ground+6, observer_x+42, ground+52, startAng=0, extent=35)
    c.setFont(FONT_BOLD, 6.0)
    c.setFillColor(Palette.PHYSICS_DARK)
    c.drawString(observer_x-18, ground+23, "observer")
    c.drawString(target_x+6, target_y-2, "target")
    c.setFont(FONT_NAME, 6.0)
    c.setFillColor(Palette.TEXT_SECONDARY)
    c.drawString(x + 22, y + 12, "The observed angle belongs to the sight line from this observer, not directly to the path range.")
...
def render_primitive_2d(kind, params, canvas, bbox):
    ...
    tracer.saveState()
    try:
        RENDERERS_2D[kind](tracer, x, y, w, h, dict(params or {}))
    finally:
        tracer.restoreState()
    return {
        "primitive_id": kind,
