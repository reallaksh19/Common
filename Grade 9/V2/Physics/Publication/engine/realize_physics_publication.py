...
WORKSPACE={'DIAGNOSTIC_PROBE_PANEL':3,'GUIDED_WORKSPACE':3,'FADED_WORKSPACE':3,'INDEPENDENT_WORKSPACE':5,'COMPLETION_EVIDENCE_PANEL':5,'BOUNDARY_STATE_TABLE':4}
EXTRA={'DIAGNOSTIC_PROBE_PANEL':42,'BOUNDARY_STATE_TABLE':54,'GUIDED_WORKSPACE':42,'FADED_WORKSPACE':42,'INDEPENDENT_WORKSPACE':70,'COMPLETION_EVIDENCE_PANEL':70,'MINIMAL_CONTRAST_PANEL':18}
...
def build_structure(product,policy):
    ...
    return {'schema_version':'1.0.0','structure_id':'PHY-PUB-STRUCTURE-SYN-001','semantic_product_ref':product['semantic_product_id'],'page_intents':intents}
...
def draw_panel(c,item,intent,y):
    ...
    for para in item['body']:
        for line in wrap(para,size=9.2,max_width=w-20):
            c.drawString(x+10,ty,line); ty-=11.5
        ty-=2.5
...
def render(product,structure,pdf):
    ...
    c.setTitle('PHY-V2-05 Physics learner candidate'); c.setAuthor('V2 deterministic publication renderer')
    for intent in structure['page_intents']:
        c.setFont('Helvetica-Bold',15.5); c.drawString(MARGIN,PAGE_H-34,intent['title'])
        c.setFont('Helvetica',7.2); c.drawRightString(PAGE_W-MARGIN,PAGE_H-32,f"PHY-V2-05 | page {intent['page_number']}")
        y=PAGE_H-53
...
def realize(product,target,policy,design_plan,out):
    ...
    page_map={'schema_version':'1.0.0','page_map_id':'PHY-PHYSICAL-PAGE-MAP-SYN-001','artifact_path':'candidate.pdf','artifact_sha256':art_sha,
              'page_count':len(structure['page_intents']),'actual_placement_evidence':True,'content_placements':placements}
    audit={'schema_version':'1.0.0','audit_id':'PHY-PUB-AUDIT-SYN-001','artifact_sha256':art_sha,
    ...
    manifest={'schema_version':'1.0.0','manifest_id':'PHY-PUB-MANIFEST-SYN-001',**desc,'package_digest':sha_obj(desc),'quality_states':quality()}
