...
S=make_session();RMAP={RECEIPT["receipt_id"]:RECEIPT};validate_session(S,POOL,RMAP,HINT)
...
transfer["event_digest"]=digest(transfer);advanced=copy.deepcopy(S);advanced["capability_bindings"][0]["transfer_events"]=[transfer]
...
current=S["retrieval_state"];new=update_retrieval_state(current,attempt);assert new["last_event_sequence"]==1 and "VY_T_GRAPH" not in new["representation_success"]

correct=copy.deepcopy(attempt);correct["event_id"]="C2B-ATTEMPT-0002";correct["event_sequence"]=2;correct["outcome"]="CORRECT";correct["event_digest"]=digest_without(correct,"event_digest");new2=update_retrieval_state(new,correct);assert new2["last_event_sequence"]==2 and "VY_T_GRAPH" in new2["representation_success"]

heavy=copy.deepcopy(correct);heavy["event_id"]="C2B-ATTEMPT-0003";heavy["event_sequence"]=3;heavy["hint_level_used"]="FULL_SOLUTION";heavy["representation_used"]="VECTOR_COMPONENTS_2D";heavy["event_digest"]=digest_without(heavy,"event_digest");new3=update_retrieval_state(new2,heavy);assert "VECTOR_COMPONENTS_2D" not in new3["representation_success"]
...
print("Physics Core2B runtime tests: PASS (13 guards + real Core2A/Core1B integration)")
