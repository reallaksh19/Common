...
from validate_engineered_domain_admission import (  # noqa: E402
    PhysicsEngineeredDomainAdmissionError,
    validate,
)

TECH_REGISTRY = load("policies/physics-technical-engineering-gates.v1.json")
...
def _asset(asset_id: str, subtopic_id: str, ref: str) -> dict:
    return {
        ...
        "payload": {
            ...
            "prerequisite_refs": [],
            "implications": ["Admission logic must not depend on a remembered physics topic."],
        },
...
def synthetic_domain_registry() -> dict:
    return {
        ...
        "subject": "PHYSICS",
        "registry_id": "PHY-REG-SYNTHETIC-001",
        ...
        "assets": [
            _asset("REG-PHY-CON-SYNTHETIC-A", "BP-SCOPE-001", "A"),
            _asset("REG-PHY-CON-SYNTHETIC-B", "BP-SCOPE-002", "B"),
        ],
    }
...
def _authorization(gate_id: str, suffix: str, consumer: str = "CANONICAL_DOMAIN_REGISTRY") -> tuple[dict, dict, dict, dict]:
    ...
    request = {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "request_id": "PHY-ENG-REQ-DOMAIN-" + suffix,
...
def runtime_bundle():
    registry = copy.deepcopy(TECH_REGISTRY)
    # Choose gates such that the second has a prerequisite starting with PHY-
    gate_1 = "PHY-VEC-BASICS"
    gate_2 = "PHY-VEC-ADD-SUB"
    gate_ids = [gate_1, gate_2]
    injected = {}
    auth_rows = []
    map_rows = []
    for i, gate_id in enumerate(gate_ids, 1):
        auth_id = f"PHY-ENG-AUTH-SYNTHETIC-{i}"
    ...
    admission = {
        ...
        "subject": "PHYSICS",
        "admission_id": "PHY-ENG-DOMAIN-ADMISSION-SYNTHETIC",
...
class EngineeredDomainAdmissionTests(unittest.TestCase):
    ...
    def test_unmapped_domain_subtopic_fails(self):
        ...
        with self.assertRaises(PhysicsEngineeredDomainAdmissionError) as ctx:
            validate(admission, domain_registry=domain, engineering_authorizations=injected, technical_registry=registry)
        self.assertEqual(ctx.exception.code, "PHY_ENG_DOMAIN_SUBTOPIC_COVERAGE_MISMATCH")

    def test_unknown_authorization_ref_fails(self):
        admission, domain, injected, registry = runtime_bundle()
        admission["subtopic_gate_map"][0]["gate_bindings"][0]["authorization_ref"] = "PHY-ENG-AUTH-NOT-PRESENT"
        with self.assertRaises(PhysicsEngineeredDomainAdmissionError) as ctx:
            validate(admission, domain_registry=domain, engineering_authorizations=injected, technical_registry=registry)
        self.assertEqual(ctx.exception.code, "PHY_ENG_DOMAIN_AUTHORIZATION_REF_UNKNOWN")
    ...
    def test_prerequisite_only_gate_cannot_be_semantic_admission_target(self):
        ...
        with self.assertRaises(PhysicsEngineeredDomainAdmissionError) as ctx:
            validate(admission, domain_registry=domain, engineering_authorizations=injected, technical_registry=registry)
        self.assertEqual(ctx.exception.code, "PHY_ENG_DOMAIN_GATE_NOT_DIRECTLY_AUTHORIZED")
