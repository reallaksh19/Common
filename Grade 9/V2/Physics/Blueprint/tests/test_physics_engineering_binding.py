...
class PhysicsEngineeringBindingTests(unittest.TestCase):
    ...
    def test_registry_change_stales_runtime_binding(self):
        ...
        with self.assertRaises(PhysicsEngineeringBindingError) as ctx:
            validate(binding, request, manifest, registry)
        self.assertIn(ctx.exception.code, {"PHY_ENG_BIND_RECEIPT_DIGEST_MISMATCH", "PHY_ENG_BIND_REGISTRY_DIGEST_MISMATCH"})
    ...
    def test_scope_change_stales_runtime_binding(self):
        ...
        with self.assertRaises(PhysicsEngineeringBindingError):
    ...
    def test_incomplete_authoritative_gate_cannot_be_consumed(self):
        ...
        with self.assertRaises(PhysicsEngineeringBindingError) as ctx:
            validate(binding, request, manifest, registry)
        self.assertEqual(ctx.exception.code, "PHY_ENG_BIND_CLOSURE_BLOCKED")
