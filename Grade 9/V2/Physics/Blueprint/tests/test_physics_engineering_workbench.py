...
def request_for_gate(gate_id: str, suffix: str = "TEST", depth: str = "STANDARD") -> dict:
    ...
    return {
        ...
        "subject": "PHYSICS",
        "request_id": f"PHY-ENG-REQ-{suffix}_{safe}",
...
def request_for_bucket(bucket_id: str, suffix: str = "BUCKET", depth: str = "STANDARD") -> dict:
    return {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "request_id": f"PHY-ENG-REQ-{suffix}",
        "scope_kind": "BUCKET",
...
class PhysicsEngineeringWorkbenchTests(unittest.TestCase):
    ...
    def test_manifest_cannot_override_registry_resolution(self):
        ...
        self.assertEqual(ctx.exception.code, "PHY_ENG_MANIFEST_STALE_OR_FORGED")
    ...
    def test_ready_gate_with_broken_engineering_checklist_is_rejected_upstream(self):
        ...
        self.assertEqual(ctx.exception.code, "PHY_ENG_REGISTRY_INVALID")
    ...
    def test_research_depth_is_a_generic_stricter_contract(self):
        ...
        self.assertIn("PHY_ENG_DEPTH_MODEL_CONDITIONS_INSUFFICIENT", state["failure_codes"])
    ...
    def test_subject_guard_rejects_non_physics_before_resolution(self):
        request = request_for_gate(gate_ids()[0], "SUBJECT")
        request["subject"] = "MATHEMATICS"
        with self.assertRaises(PhysicsEngineeringWorkbenchError) as ctx:
            resolve_manifest(request, copy.deepcopy(REGISTRY))
        self.assertEqual(ctx.exception.code, "PHY_ENG_REQUEST_SCHEMA")

    def test_dependency_cycle_is_rejected_by_upstream_engineering_validation(self):
        registry = copy.deepcopy(REGISTRY)
        child = next(
            (g for g in registry["subtopic_gates"] if any(p.startswith("PHY-") for p in g.get("prerequisite_ids", []))),
            None
        )
        if child is None:
            self.skipTest("registry contains no prerequisite edges")
        parent_id = next(p for p in child["prerequisite_ids"] if p.startswith("PHY-"))
