...
def validate():
    ...
    if a['role_lifecycle']['CORE2A']!='ACTIVE' or not a['invariants']['core2a_requires_taught_state_receipts'] or not a['invariants']['core2a_may_not_infer_learner_mastery']:raise AssertionError('CORE2A_BLUEPRINT_GUARD_DRIFT')
    if a['invariants']['max_subtopics_per_handoff']!=3 or not a['invariants']['learning_atoms_unbounded']:raise AssertionError('TRANSPORT_PEDAGOGY_BOUND_DRIFT')
    ...
    independent=load(ROOT/'policy'/'independent-validation.v1.json');join=load(ROOT/'policy'/'join-policy.v1.json');purpose=load(ROOT/'policy'/'purpose-contracts.v1.json');c1a=load(ROOT/'policy'/'core1a-stage-machine.v1.json');pub=load(ROOT/'policy'/'publication-boundary.v1.json')
