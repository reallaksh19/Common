#!/usr/bin/env python3
"""Tests for PhysicsBlueprint Run Builder Configuration Compiler."""
...
FIXTURE_PATH = TOOL_DIR / "fixtures" / "cbse_work_energy_fixture.json"


class PhysicsRunBuilderTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_complete_configuration_compiles_successfully(self):
        ...
        self.assertIn("Work, Kinetic Energy", prompt)
    ...
    def test_unsupported_grade_fails(self):
        ...
        manifest = compile_manifest(cfg)
        self.assertEqual(manifest["validation"]["status"], "INVALID")
        self.assertTrue(any(e["code"] == "INVALID_GRADE" and e["field"] == "current_grade" for e in manifest["validation"]["errors"]))

    def test_owner_notes_do_not_silently_override_structured_fields(self):
        cfg = copy.deepcopy(self.base_config)
        cfg["owner_scope_notes"] = "Please treat depth as RESEARCH and grade as 11."
        manifest = compile_manifest(cfg)
        self.assertEqual(manifest["engineering"]["requested_depth"], "STANDARD")
        self.assertEqual(manifest["request"]["current_grade"], 9)
