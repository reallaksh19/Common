#!/usr/bin/env python3
"""Project validated Physics Engineering authority into Canonical Domain assets.
...
from compile_physics_engineering_workbench import digest as engineering_digest
from validate_canonical_domain_registry import validate_registry

HERE = Path(__file__).resolve()
ROOT = HERE.parents[1]
PROJECTION_SCHEMA = ROOT / "contracts" / "physics-engineering-domain-projection.schema.json"
...
def _asset_id(kind: str, subtopic_id: str, gate_id: str, source_ref: str) -> str:
    ...
    return f"REG-PHY-ENG-{kind}-{digest(material)[:16].upper()}"
...
def _closure(gates: dict[str, dict], gate_id: str, cache: dict[str, list[str]], visiting: set[str] | None = None) -> list[str]:
    ...
    for prereq in gate.get("prerequisite_ids", []):
        if not isinstance(prereq, str) or not prereq.startswith("PHY-"):
            continue
...
PHYSICS_REPRESENTATION_TYPES = {
    "FREE_BODY_DIAGRAM",
    "GEOMETRIC_VECTOR_DIAGRAM",
    "COORDINATE_SYSTEM",
    "CARTESIAN_COMPONENT_DECOMPOSITION",
    "SCHEMATIC_SYSTEM_BOUNDARY",
    "ALGEBRAIC_SUPERPOSITION",
    "CONSTRAINT_DIAGRAM",
...
def _representation_type(value: str) -> str:
    ...
    if text in PHYSICS_REPRESENTATION_TYPES:
        return text
    if "FREE_BODY" in text or "FBD" in text:
        return "FREE_BODY_DIAGRAM"
    if "RAY" in text or "OPTIC" in text:
        return "RAY_OPTICS_DIAGRAM"
    if "CIRCUIT" in text or "SCHEMATIC" in text:
        return "CIRCUIT_SCHEMATIC"
    if "PV" in text or "THERMODYNAMIC" in text or "CYCLE" in text:
        return "THERMODYNAMIC_PV_CYCLE"
    if "WAVE" in text or "OSCILLAT" in text:
        return "WAVE_OSCILLATION_PLOT"
    if "FIELD" in text or "EQUIPOTENTIAL" in text:
        return "FIELD_LINE_EQUIPOTENTIAL_MAP"
    if "VECTOR" in text:
        return "GEOMETRIC_VECTOR_DIAGRAM"
    if "COORDINATE" in text:
        return "COORDINATE_SYSTEM"
    if "GRAPH" in text or "PLOT" in text:
        return "KINEMATIC_GRAPH_PLOT"
    if "TABLE" in text or "CHART" in text:
        return "TABLE"
    if "FLOW" in text or "DEPENDENCY" in text:
...
def _verification_type(text: str) -> str:
    value = text.lower()
    if "dimension" in value or "unit" in value:
    ...
    if any(token in value for token in ("order of magnitude", "magnitude check", "scale")):
        return "ORDER_OF_MAGNITUDE"
    if any(token in value for token in ("conservation", "energy balance", "momentum balance")):
        return "CONSERVATION_CHECK"
    if any(token in value for token in ("geometry", "geometric", "distance", "angle")):
        return "GEOMETRIC_CONSISTENCY"
    if any(token in value for token in ("identity", "expand", "factor")):
...
def _common(
    *,
    ...
    asset = {
        ...
        "payload": payload,
        "provenance_note": (
            f"Deterministic {gate_role} projection from Physics Engineering Gate {gate_id}; "
...
def project_engineering_to_domain_registry(
    ...
    *,
        ...
        for gate_id in bucket_closure:
            ...
            for index, row in enumerate(gate.get("mandatory_equations", []), 1):
                ...
                for symbol in row.get("symbols", []):
                    ...
                    si_unit = symbol.get("si_unit", "")
                    dim = symbol.get("dimension", "")
                    domain = symbol.get("domain", "")
                    role = symbol.get("geometric_or_algebraic_role", "")
                    parts = [f"{sym}: {name}"]
                    if si_unit:
                        parts.append(f"si_unit={si_unit}")
                    if dim:
                    ...
                    term_meanings.append("; ".join(parts))
            ...
            physics_bindings = concept_ids + equation_ids
            for index, row in enumerate(gate.get("representations", []), 1):
                ...
                encoded = row.get("physics_encoded") or row.get("math_encoded", "")
                if str(encoded).strip():
                    must_visible.append(str(encoded))
                must_not = []
                ...
                emit(
                    ...
                    source_ref=source_ref, gate_role=gate_role, depends_on=physics_bindings,
                    payload={
                        ...
                        "physics_bindings": physics_bindings,
                        "math_bindings": physics_bindings,
                    },
                )
    ...
    receipt["projection_id"] = "PHY-ENG-DOMAIN-PROJ-" + digest(identity)[:16].upper()
