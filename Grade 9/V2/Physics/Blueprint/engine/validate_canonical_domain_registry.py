...
PHYSICS_BLUEPRINT = HERE.parents[1]
DEFAULT_SCHEMA = PHYSICS_BLUEPRINT / "contracts" / "physics-canonical-domain-registry.schema.json"
DEFAULT_POLICY = PHYSICS_BLUEPRINT / "policies" / "physics-canonical-domain-registry-policy.json"


def load(path: str | Path) -> dict:
    target = Path(path)
    if not target.is_absolute():
        target = PHYSICS_BLUEPRINT / target
...
def _internal_refs(asset: dict) -> set[str]:
    ...
    for key, value in payload.items():
        if key.endswith("_ref") and isinstance(value, str) and value.startswith("REG-PHY-"):
            refs.add(value)
        elif key.endswith("_refs") and isinstance(value, list):
            refs.update(x for x in value if isinstance(x, str) and x.startswith("REG-PHY-"))
    return refs
