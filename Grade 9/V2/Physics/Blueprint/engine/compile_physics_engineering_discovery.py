...
REGISTRY_REL = "policy/physics-technical-engineering-gates.v1.json"
VOCABULARY_REL = "policy/physics-engineering-discovery-vocabulary.v1.json"
VOCABULARY_SCHEMA = "physics-engineering-discovery-vocabulary.schema.json"


class PhysicsEngineeringDiscoveryError(Exception):
...
def _schema_validate(doc: dict, schema_name: str, code: str) -> None:
    ...
    if errors:
        e = errors[0]
        raise PhysicsEngineeringDiscoveryError(code, f"{e.message}; path={list(e.path)}")
...
def validate_discovery_vocabulary_catalog(catalog: dict, registry: dict) -> dict:
    _schema_validate(catalog, VOCABULARY_SCHEMA, "PHY_ENG_DISCOVERY_VOCABULARY_SCHEMA")
    if catalog["registry_id"] != registry.get("registry_id"):
        raise PhysicsEngineeringDiscoveryError(
            "PHY_ENG_DISCOVERY_VOCABULARY_REGISTRY_ID_MISMATCH",
    ...
    for entry in catalog["entries"]:
        target_ref = entry["target_scope_ref"]
        if target_ref in seen_targets:
            raise PhysicsEngineeringDiscoveryError(
                "PHY_ENG_DISCOVERY_VOCABULARY_DUPLICATE_TARGET", f"duplicate vocabulary target {target_ref}"
            )
        seen_targets.add(target_ref)
        if target_ref not in gate_ids:
            raise PhysicsEngineeringDiscoveryError(
                "PHY_ENG_DISCOVERY_VOCABULARY_UNKNOWN_TARGET", f"unknown vocabulary target {target_ref}"
            )
        ...
        for term in entry["terms"]:
            ...
            if not normalized:
                raise PhysicsEngineeringDiscoveryError(
                    "PHY_ENG_DISCOVERY_VOCABULARY_EMPTY_TERM", f"empty normalized term for {target_ref}"
                )
            if normalized in seen_terms:
                raise PhysicsEngineeringDiscoveryError(
                    "PHY_ENG_DISCOVERY_VOCABULARY_DUPLICATE_TERM",
                    f"duplicate normalized term {normalized!r} for {target_ref}",
                )
...
def _load_vocabulary_catalog() -> dict:
    ...
    if not vocab_path.exists():
        vocab_path = ROOT / "policies" / "physics-engineering-discovery-vocabulary.v1.json"
    ...
    for index, raw in enumerate([query, *hints]):
        ...
        if best_overlap:
            ...
            for phrase, _, toks in vocabulary_rows:
                if len(term_tokens & toks) == best_overlap:
                    _add_once(matched, phrase)
...
def discover_candidates(request: dict, registry: dict | None = None, vocabulary_catalog: dict | None = None) -> dict:
    _schema_validate(request, "physics-engineering-discovery-request.schema.json", "PHY_ENG_DISCOVERY_REQUEST_SCHEMA")
    registry = registry or load_engineering(REGISTRY_REL)
    if registry.get("registry_id") != "PHY-G9-11-TECHNICAL-ENGINEERING-GATES-v1":
        raise PhysicsEngineeringDiscoveryError("PHY_ENG_DISCOVERY_REGISTRY_ID_MISMATCH", str(registry.get("registry_id")))
    ...
    suffix = request["discovery_request_id"].removeprefix("PHY-ENG-DISC-REQ-")
    receipt = {
        "schema_version": "1.1.0",
        "subject": "PHYSICS",
        "discovery_id": f"PHY-ENG-DISC-{suffix}",
    ...
    _schema_validate(receipt, "physics-engineering-discovery-receipt.schema.json", "PHY_ENG_DISCOVERY_RECEIPT_SCHEMA")
    return receipt
...
def _verify_receipt(request: dict, receipt: dict, registry: dict, vocabulary_catalog: dict) -> None:
    ...
    if receipt != discover_candidates(request, registry, vocabulary_catalog):
        raise PhysicsEngineeringDiscoveryError(
            "PHY_ENG_DISCOVERY_RECEIPT_STALE_OR_FORGED",
...
def promote_explicit_selection(
    ...
    _schema_validate(selection, "physics-engineering-discovery-selection.schema.json", "PHY_ENG_DISCOVERY_SELECTION_SCHEMA")
    registry = registry or load_engineering(REGISTRY_REL)
    vocabulary_catalog = vocabulary_catalog or _load_vocabulary_catalog()
    ...
    if selection["discovery_id"] != discovery_receipt["discovery_id"]:
        raise PhysicsEngineeringDiscoveryError("PHY_ENG_DISCOVERY_SELECTION_WRONG_RECEIPT", selection["discovery_id"])
    if selection["discovery_receipt_digest"] != digest(discovery_receipt):
        raise PhysicsEngineeringDiscoveryError("PHY_ENG_DISCOVERY_SELECTION_DIGEST_MISMATCH", "selection is not bound to this exact discovery receipt")
    ...
    if missing:
        raise PhysicsEngineeringDiscoveryError(
            "PHY_ENG_DISCOVERY_SELECTION_NOT_CANDIDATE",
            f"selected exact identities were not in the bound candidate set: {missing}",
        )
    suffix = selection["selection_id"].removeprefix("PHY-ENG-DISC-SEL-")
    request = {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "request_id": f"PHY-ENG-REQ-DISC_{suffix}",
    ...
    _schema_validate(request, "physics-engineering-request.schema.json", "PHY_ENG_DISCOVERY_PROMOTED_REQUEST_SCHEMA")
    return request


def main() -> None:
    parser = argparse.ArgumentParser(description="Compile non-authoritative Physics Engineering discovery candidates")
