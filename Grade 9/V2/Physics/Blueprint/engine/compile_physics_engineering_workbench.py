...
REGISTRY_REL = "policies/physics-technical-engineering-gates.v1.json"
ENGINEERING_SCHEMA_REL = "contracts/physics-technical-engineering-gate.schema.json"
ENGINEERING_VALIDATOR_PATH = ROOT / "engine" / "validate_engineering_gates.py"
ENGINEERING_COMPOSER_PATH = ROOT / "engine" / "engineering_registry_composition.py"
ENGINEERING_INVARIANT_PROFILE_PATH = ROOT / "policies" / "physics-engineering-gate-invariants.v1.json"
ENGINEERING_DEPTH_POLICY_PATH = ROOT / "policies" / "physics-engineering-depth-policy.v1.json"
...
from validate_engineering_gates import (  # noqa: E402
    EngineeringGateValidationError as PhysicsEngineeringGateValidationError,
    validate as validate_engineering_registry,
)


class PhysicsEngineeringWorkbenchError(Exception):
...
def schema_validate(doc: dict, schema_name: str, code: str) -> None:
    ...
    if errors:
        e = errors[0]
        raise PhysicsEngineeringWorkbenchError(code, f"{e.message}; path={list(e.path)}")


def _validate_authoritative_registry(registry: dict) -> None:
    if registry.get("registry_id") not in ("PHY-G9-11-TECHNICAL-ENGINEERING-GATES-v1", "REG-PHY-TECH-GATE-V1"):
        raise PhysicsEngineeringWorkbenchError(
            "PHY_ENG_REGISTRY_ID_MISMATCH",
            f"expected PHY-G9-11-TECHNICAL-ENGINEERING-GATES-v1, got {registry.get('registry_id')}",
        )
    try:
        validate_engineering_registry(registry)
    except PhysicsEngineeringGateValidationError as exc:
        raise PhysicsEngineeringWorkbenchError("PHY_ENG_REGISTRY_INVALID", str(exc)) from exc
...
def _load_depth_policy() -> dict:
    ...
    if (
        ...
        or policy.get("subject") != "PHYSICS"
        or set(policy.get("profiles", {})) != {"FOUNDATION", "STANDARD", "RESEARCH"}
    ):
        raise PhysicsEngineeringWorkbenchError("PHY_ENG_DEPTH_POLICY_INVALID", "top-level policy shape")
...
def _depth_failures(gate: dict, engineering_depth: str) -> list[str]:
    ...
    if profile is None:
        raise PhysicsEngineeringWorkbenchError(
            "PHY_ENG_DEPTH_UNKNOWN",
            engineering_depth,
        )

    failures: list[str] = []
    checks = (
        ("reasoning_sequence", "min_reasoning_steps", "PHY_ENG_DEPTH_REASONING_INSUFFICIENT"),
        ("model_conditions", "min_model_conditions", "PHY_ENG_DEPTH_MODEL_CONDITIONS_INSUFFICIENT"),
        ("misconceptions", "min_misconceptions", "PHY_ENG_DEPTH_MISCONCEPTION_COVERAGE_INSUFFICIENT"),
        ("required_transformations", "min_required_transformations", "PHY_ENG_DEPTH_TRANSFORMATIONS_INSUFFICIENT"),
        ("falsification_cases", "min_falsification_cases", "PHY_ENG_DEPTH_FALSIFICATION_INSUFFICIENT"),
    )
    ...
    if not required_roles.issubset(actual_roles):
        failures.append("PHY_ENG_DEPTH_TRANSFORMATION_COVERAGE_INSUFFICIENT")

    if profile["require_non_provisional_provenance"]:
        if gate.get("provenance", {}).get("claim_status") == "PROVISIONAL":
            failures.append("PHY_ENG_DEPTH_PROVENANCE_PROVISIONAL")
...
def resolve_manifest(request: dict, registry: dict | None = None) -> dict:
    ...
    """
    schema_validate(request, "physics-engineering-request.schema.json", "PHY_ENG_REQUEST_SCHEMA")
    if request["subject"] != "PHYSICS":
        raise PhysicsEngineeringWorkbenchError("PHY_ENG_SUBJECT_MISMATCH", "request is not PHYSICS")
    ...
    if request["scope_kind"] == "ENGINEERING_GATE":
        ...
        raise PhysicsEngineeringWorkbenchError("PHY_ENG_SCOPE_KIND_INVALID", request["scope_kind"])

    suffix = request["request_id"].removeprefix("PHY-ENG-REQ-")
    manifest = {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "manifest_id": f"PHY-ENG-MANIFEST-{suffix}",
    ...
    schema_validate(manifest, "physics-engineering-manifest.schema.json", "PHY_ENG_MANIFEST_SCHEMA")
    return manifest
...
def _closure(gates: dict[str, dict], direct_gate_ids: list[str]) -> list[str]:
    ...
    def walk(gate_id: str) -> None:
        ...
        if gate_id in visiting:
            raise PhysicsEngineeringWorkbenchError("PHY_ENG_DEPENDENCY_CYCLE", gate_id)
        gate = gates.get(gate_id)
        if gate is None:
            raise PhysicsEngineeringWorkbenchError("PHY_ENG_GATE_UNKNOWN", gate_id)
...
def authoritative_gate_state(gate: dict, engineering_depth: str = "STANDARD") -> dict:
    ...
    if gate.get("authority_tier") == "SOURCE_SCOPE_HELD" or source_scope == "HELD_SCOPE":
        failures.append("PHY_ENG_SOURCE_SCOPE_HELD")
    if readiness != "ENGINEERING_GATE_READY":
        failures.append("PHY_ENG_GATE_NOT_READY")
...
def _verify_manifest_against_resolution(request: dict, manifest: dict, registry: dict) -> None:
    ...
    for field in fields:
        if manifest.get(field) != expected.get(field):
            raise PhysicsEngineeringWorkbenchError(
                "PHY_ENG_MANIFEST_STALE_OR_FORGED",
...
def compile_closure(
    ...
    receipt = {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "receipt_id": f"PHY-ENG-CLOSURE-{suffix}",
        "request_id": request["request_id"],
    ...
    schema_validate(receipt, "physics-engineering-closure-receipt.schema.json", "PHY_ENG_RECEIPT_SCHEMA")
    return receipt
...
def _engineering_surface(request: dict, receipt: dict, registry: dict) -> dict:
    if digest(registry) != receipt["registry_digest"]:
        raise PhysicsEngineeringWorkbenchError(
            "PHY_ENG_PASSPORT_REGISTRY_CUSTODY_DRIFT",
...
def compile_passport(
    ...
    return {
        ...
        "subject": "PHYSICS",
        "passport_id": "PHY-ENG-PASSPORT-" + request["request_id"].removeprefix("PHY-ENG-REQ-"),
        "request_id": request["request_id"],
        "scope_kind": request["scope_kind"],
        "scope_refs": list(request["scope_refs"]),
...
def compile_binding(
    ...
    binding = {
        "schema_version": "1.0.0",
        "subject": "PHYSICS",
        "binding_id": f"PHY-ENG-BIND-{suffix}",
    ...
    schema_validate(binding, "physics-engineering-binding.schema.json", "PHY_ENG_BIND_SCHEMA")
    return binding
...
def compile_registry_proof(registry: dict | None = None) -> dict:
    ...
    for gate in registry["subtopic_gates"]:
        ...
        request = {
            "schema_version": "1.0.0",
            "subject": "PHYSICS",
            "request_id": f"PHY-ENG-REQ-REGISTRY-PROOF-{gate_id}",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Compile registry-driven Physics Engineering authorization")
