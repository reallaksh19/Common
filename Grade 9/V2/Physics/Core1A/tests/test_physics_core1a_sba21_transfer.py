...
PROFILE = json.loads((ROOT / 'registry' / 'physics-core1a-motion-in-a-plane-sba21-20pct-v1.json').read_text(encoding='utf-8'))
TRANSFER = json.loads((ROOT / 'registry' / 'physics-core1a-motion-in-a-plane-sba21-transfer-v1.json').read_text(encoding='utf-8'))
MANIFEST = json.loads((ROOT / 'registry' / 'build-manifests' / 'M2D-SBA-21-v1.json').read_text(encoding='utf-8'))
...
canonical = next(b for b in REGISTRY['buckets'] if b['bucket_id']=='M2D-SBA-21')
assert canonical['intrinsic_difficulty']=='D3'
assert canonical['source_basis']==['SRC-GFORM','SRC-GPRACTICE']
assert canonical['core2_primary_questions']==['Q54','Q55','Q56']
assert canonical['prerequisite_buckets']==['M2D-SBA-03','M2D-SBA-06']

bucket = PROFILE['buckets'][0]
assert bucket['bucket_id']=='M2D-SBA-21'
assert bucket['core2_primary_questions']==['Q54','Q55','Q56']
p = bucket['profiles'][0]
assert p['prior_knowledge_pct']==20 and p['pathway']=='FOUNDATION_PATH'
assert len(p['learning_atoms'])==9
assert max(a['visual_stage_count'] for a in p['learning_atoms']) >= 6
assert {a['atom_id'] for a in p['learning_atoms']} == {f'M2D-SBA-21{c}' for c in 'ABCDEFGHI'}
coverage={q['question_id']:q for q in p['core2_hint_coverage']}
assert set(coverage)=={'Q54','Q55','Q56'}
...
assert set(routines)=={'M2D-SBA-21-R1','M2D-SBA-21-R2','M2D-SBA-21-R3'}
assert routines['M2D-SBA-21-R1']['core2_questions']==['Q54']
assert routines['M2D-SBA-21-R2']['core2_questions']==['Q55']
assert routines['M2D-SBA-21-R3']['core2_questions']==['Q56']
...
assert MANIFEST['primary_questions']==['Q54','Q55','Q56']
assert MANIFEST['question_load']=={'count':3,'class':'MEDIUM','minimum_transfer_families':3}
assert {q['question_id'] for q in MANIFEST['question_release']}=={'Q54','Q55','Q56'}
...
assert rows['M2D-SBA-21']['core2_primary_questions']==['Q54','Q55','Q56']
assert rows['M2D-SBA-21']['core1a_teaching_home']==['PROJECTILE_INCLINED_PLANE']
assert rows['M2D-SBA-21']['origin']=='Formula sheet + Level-I practice'
assert rows['M2D-SBA-21']['state'] in {'IN_PROGRESS','BUILT'}
...
assert 'formula/practice' in notes and 'does not contain detailed inclined-plane theory' in notes

print('SBA21 inclined-plane transfer and professional-lossless checks passed.')
