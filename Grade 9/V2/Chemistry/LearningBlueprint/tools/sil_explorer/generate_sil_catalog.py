...
GATES_PATH = ROOT / "policies" / "chemistry-technical-engineering-gates.v1.json"
...
def parse_packet_details(packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    domain = meta.get("chapter", "Chemistry Foundation")
    grade_val = meta.get("cbse_ref", {}).get("grade", 10)
    grade_level = f"Grade {grade_val}"
    tier = meta.get("jee_tier", "BOTH")
    exam_families = ["CBSE", "JEE Main"]
    if tier in ("JEE_ADVANCED", "OLYMPIAD"):
        exam_families.extend(["JEE Advanced", "Olympiad"])
    ...
    for tm in ttu_matches:
        ...
        ttu_role = tm.group(3).strip() if tm.group(3) else "SCAFFOLD"
        ttu_content = tm.group(4)
        scaffold_m = re.search(r"\[INCOMPLETE STATE.*?\](.*?)(?=\[COMPLETION|\Z)", ttu_content, re.DOTALL)
        key_m = re.search(r"\[COMPLETION KEY.*?\](.*?)(?=```|\n####|\n###|\Z)", ttu_content, re.DOTALL)
        ttus.append({
            "ttu_id": ttu_id,
            "title": ttu_title,
            "role": ttu_role,
            "scaffold": scaffold_m.group(1).strip() if scaffold_m else "",
            "completion_key": key_m.group(1).strip() if key_m else "",
        })
...
def main() -> None:
    ...
    js_content = f"// Subtopic Intelligence Library - Chemistry Catalog Data\nwindow.SIL_CATALOG = {serialized};\n"
    OUTPUT_JS.write_text(js_content, encoding="utf-8")
    print(f"Generated Chemistry SIL catalog data at {OUTPUT_JS} ({len(catalog)} packets)")
