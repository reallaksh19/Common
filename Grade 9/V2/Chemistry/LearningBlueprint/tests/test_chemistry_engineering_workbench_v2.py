...
def make_production_audit(gate):
    bindings = []
    for concept_id in gate["canonical_concept_ids"]:
        bindings.append({"asset_kind":"CONCEPT","asset_ref":concept_id,"authority_layer_ids":["SRC-TEST-AUTHORITY"],"scope_tier_id":"TEST_TIER","claim_class":"SOURCE_DEFINED","downstream_scope":"TEST_ONLY source-scope exercise"})
    for equation in gate["mandatory_equations"]:
        bindings.append({"asset_kind":"EQUATION","asset_ref":equation["equation_id"],"authority_layer_ids":["SRC-TEST-AUTHORITY"],"scope_tier_id":"TEST_TIER","claim_class":"SOURCE_DEFINED","downstream_scope":"TEST_ONLY source-scope exercise"})
    for transform in gate["required_transformations"]:
        bindings.append({"asset_kind":"TRANSFORMATION","asset_ref":transform_ref(transform),"authority_layer_ids":["SRC-TEST-AUTHORITY"],"scope_tier_id":"TEST_TIER","claim_class":"SOURCE_DEFINED","downstream_scope":"TEST_ONLY source-scope exercise"})
    requirements = [kind for kind, values in (
        ("CONCEPT", gate["canonical_concept_ids"]),
        ("EQUATION", gate["mandatory_equations"]),
        ("TRANSFORMATION", gate["required_transformations"]),
    ) if values]
    return {
        "schema_version":"2.0.0",
        "audit_id":"CHEM-SOURCE-AUDIT-GENERIC-TEST-V2",
        "gate_id":gate["subtopic_id"],
        "base_registry_ref":"policies/chemistry-technical-engineering-gates.v1.json",
        "audit_role":"PRODUCTION_SOURCE_AUDIT",
...
def make_request(gate):
    return {
        "schema_version":"2.0.0",
        "request_id":"CHEM-ENG-REQ-GENERIC-TEST",
        "subject":"CHEMISTRY",
        "requested_topic":"Generic test scope",
        "requested_scope":"One declared direct gate plus derived prerequisites",
...
def make_manifest(gate, audit_ref="tests/in-memory-production-audit.json"):
    ...
    return {
        "schema_version":"2.0.0",
        "manifest_id":"CHEM-ENG-MAN-GENERIC-TEST",
        "request_id":"CHEM-ENG-REQ-GENERIC-TEST",
        "scope_kind":"SUBTOPIC",
        "scope_ref":"TEST_ONLY",
        "topic_id":"CHEM-GENERIC-TEST",
        "title":"Generic Workbench test",
        "registry_ref":"policies/chemistry-technical-engineering-gates.v1.json",
        "required_gate_ids":[gate["subtopic_id"]],
        "optional_gate_ids":[],
        "out_of_scope_gate_ids":[],
        "source_audits":[{"gate_id":gate["subtopic_id"],"audit_ref":audit_ref}],
        "external_prerequisite_resolutions":external,
        "source_item_status":"INDEPENDENT_OF_TECHNICAL_GATE",
        "downstream_consumers":["CDAU","PAL"],
    }
...
class ChemistryEngineeringWorkbenchV2Tests(unittest.TestCase):
    ...
    def test_direct_gate_closure_and_binding_are_generic(self):
        ...
        binding = compile_binding(self.request, self.manifest, registry=REGISTRY, source_audit_payloads=self.audit_payloads)
        self.assertEqual(binding["status"], "ENGINEERING_AUTHORIZED")
        passport = compile_passport(self.request, receipt)
    ...
    def test_source_audit_must_be_explicit_not_topic_triggered(self):
        ...
        self.assertEqual(receipt["counts"]["source_audit_count"], 0)
    ...
    def test_research_depth_still_fails_closed_without_evidence(self):
        request = copy.deepcopy(self.request)
        request["engineering_depth"] = "RESEARCH"
        ...
        self.assertIn("CHEM_ENG_RESEARCH_DOSSIER_REQUIRED", codes)
        self.assertIn("CHEM_ENG_CLAIM_LEDGER_REQUIRED", codes)
    ...
    def test_request_schema_does_not_claim_auto_discovery(self):
        ...
        bad["requested_action"] = "AUTO_DISCOVER"
    ...
    def test_stress_audit_cannot_authorize_product_scope(self):
        ...
        scope = {
            "schema_version":"2.0.0",
            "scope_contract_id":"CHEM-PRODUCT-SCOPE-STRESS-NEGATIVE",
            "gate_id":STRESS_AUDIT["gate_id"],
            "subtopic_id":"TEST-SUBTOPIC",
            "source_audit_ref":"stress_tests/redox/source-audit.v2.json",
            "source_audit_digest":digest(STRESS_AUDIT),
            "learner_grade":11,
            "program_context":"GRADE_LEVEL",
            "authorized_scope_tiers":[x["tier_id"] for x in STRESS_AUDIT["scope_tiers"]],
            "held_scope_tiers":[],
            "extension_authority_ref":"",
            "extension_reason":"",
            "learning_atom_scope_assignments":[{"learning_atom_id":"ATOM-1","scope_tier":STRESS_AUDIT["scope_tiers"][0]["tier_id"]}],
            "held_transformation_guards":[],
            "status":"SCOPE_CONTRACT_READY",
        }
        with self.assertRaises(ChemistryProductSourceScopeError) as ctx:
            validate_product_source_scope(scope, STRESS_AUDIT, authority, REGISTRY)
        self.assertEqual(ctx.exception.code, "CHEM_PRODUCT_SCOPE_STRESS_AUDIT_NOT_AUTHORITY")

    def test_production_audit_can_authorize_generic_product_scope(self):
        authority = {"subtopic_id":"TEST-SUBTOPIC","learning_atoms":[{"atom_id":"ATOM-1"}],"content_objects":[]}
        scope = {
            "schema_version":"2.0.0",
            "scope_contract_id":"CHEM-PRODUCT-SCOPE-GENERIC-TEST",
            "gate_id":self.audit["gate_id"],
            "subtopic_id":"TEST-SUBTOPIC",
            "source_audit_ref":"tests/in-memory-production-audit.json",
            "source_audit_digest":digest(self.audit),
            "learner_grade":9,
            "program_context":"GRADE_LEVEL",
            "authorized_scope_tiers":["TEST_TIER"],
            "held_scope_tiers":[],
            "extension_authority_ref":"",
            "extension_reason":"",
            "learning_atom_scope_assignments":[{"learning_atom_id":"ATOM-1","scope_tier":"TEST_TIER"}],
            "held_transformation_guards":[],
            "status":"SCOPE_CONTRACT_READY",
        }
        result = validate_product_source_scope(scope, self.audit, authority, REGISTRY)
    ...
    def test_generic_workbench_engines_have_no_redox_branching(self):
        names = [
            "validate_chemistry_engineering_registry_runtime.py",
            "compile_chemistry_engineering_closure.py",
            "compile_chemistry_engineering_authorization_binding.py",
            "compile_chemistry_engineering_passport.py",
            "validate_product_source_scope.py",
        ]
        for name in names:
