#!/usr/bin/env python3
"""
Unit Tests for Chemistry Subtopic Intelligence Library (SIL) Intake Validator
=============================================================================
...
class ChemistrySubtopicIntelligenceLibraryTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_sil_intake_validation_full_spec_passes(self) -> None:
        ...
        self.assertEqual(report["total_packets_evaluated"], 52)
        self.assertEqual(report["packets_passing"], 52)
    ...
    def test_sil_validator_detects_insufficient_atoms(self) -> None:
        packets = extract_packets(self.spec_text)
        p = dict(packets[0])
        p["body"] = re.sub(r"ATOM-[A-Z0-9]+-\d+", "CONCEPT_ITEM", p["body"])
        result = validate_packet(p)
        self.assertEqual(result["status"], "FAIL")
    ...
    def test_sil_validator_detects_missing_exam_families(self) -> None:
        packets = extract_packets(self.spec_text)
        p = dict(packets[0])
        p["body"] = re.sub(r"FAMILY-[A-Z0-9]+-\d+", "EXAM_ITEM", p["body"])
        result = validate_packet(p)
        self.assertEqual(result["status"], "FAIL")
