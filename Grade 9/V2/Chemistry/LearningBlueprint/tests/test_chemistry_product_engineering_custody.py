...
custody = compile_product_custody(REQUEST, MANIFEST, CCBOM)
...
result = validate_pal_engineering_ready(REQUEST, MANIFEST, CCBOM, custody)
...
try:
    compile_product_custody(REQUEST, bad_manifest, CCBOM)
...
try:
    compile_product_custody(REQUEST, MANIFEST, bad_ccbom)
except ChemistryProductEngineeringCustodyError as exc:
    assert exc.code == "CHEM_PRODUCT_ENGINEERING_SCOPE_MISMATCH"
else:
    raise AssertionError("CCBOM scope drift must fail")

for field in ["ccbom_digest", "engineering_binding_digest", "closure_digest", "registry_digest"]:
    stale = copy.deepcopy(custody)
    stale[field] = "sha256:" + "0" * 64
    try:
        validate_pal_engineering_ready(REQUEST, MANIFEST, CCBOM, stale)
