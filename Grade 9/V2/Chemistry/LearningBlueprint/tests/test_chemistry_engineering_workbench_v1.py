...
for s in ["chemistry-engineering-request.schema.json","chemistry-engineering-topic-manifest.schema.json","chemistry-engineering-research-dossier.schema.json","chemistry-engineering-claim-ledger.schema.json","chemistry-engineering-closure-receipt.schema.json","chemistry-engineering-passport.schema.json"]: Draft202012Validator.check_schema(load("contracts/"+s))
...
result=validate_engineered_cdau(REQUEST,MANIFEST,CDAU); assert result["status"]=="PASS"; assert result["engineering_gate_count"]==5
