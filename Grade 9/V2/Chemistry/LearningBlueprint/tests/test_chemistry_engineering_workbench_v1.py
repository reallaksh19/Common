...
assert set(receipt["closure_gate_ids"])==EXPECTED
assert receipt["counts"]["closure_gate_count"]==5
assert receipt["counts"]["external_dependency_count"]==1
assert receipt["external_dependency_states"][0]["dependency_id"]=="MATH-BASIC-ARITHMETIC"
assert receipt["external_dependency_states"][0]["status"]=="RESOLVED_BY_OWNER_SCOPE"
assert receipt["source_item_status"]=="SOURCE_HELD"
assert len(receipt["source_audit_states"])==1
sa=receipt["source_audit_states"][0]
...
assert passport["cdau_technical_authorization"]=="ALLOWED"
assert passport["source_audit_states"]==receipt["source_audit_states"]

result=validate_engineered_cdau(REQUEST,MANIFEST,CDAU)
assert result["status"]=="PASS"
assert result["engineering_binding_id"]=="CHEM-ENG-BIND-REDOX-001"
assert result["engineering_binding_digest"].startswith("sha256:")
assert result["engineering_closure_digest"]==receipt["closure_digest"]
assert result["source_audit_states"]==receipt["source_audit_states"]
...
tampered=copy.deepcopy(receipt); tampered["source_audit_states"]=[]; must_passport_fail(REQUEST,tampered,"CHEM_PASS_RECEIPT_INCONSISTENT")
