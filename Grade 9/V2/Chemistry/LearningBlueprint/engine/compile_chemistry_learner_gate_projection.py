...
def _gate_from_atom(atom: dict[str, Any], rule: dict[str, Any]) -> dict[str, Any]:
    return {
        "learner_gate_id": learner_gate_id(atom["semantic_id"], rule["learner_job"], rule["polarity"]),
...
def compile_learner_gate_projection(
    ...
    *,
    ...
    governed_semantics = copy.deepcopy(semantic_projection) if semantic_projection is not None else compile_semantic_projection(obligation_packet)
    try:
    ...
    resolved_projection_id = projection_id or governed_semantics["projection_id"].replace("CHEM-BP-SEM-", "CHEM-BP-LG-", 1)
    output = {
...
def validate_learner_gate_projection(
    ...
    *,
    ...
    expected = compile_learner_gate_projection(obligation_packet, semantic_projection=semantic_projection, policy=policy, projection_id=projection["projection_id"])
