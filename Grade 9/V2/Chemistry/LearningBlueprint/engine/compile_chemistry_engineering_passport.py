...
def validate_receipt(r):
    ...
    if "CHEM-REDOX-OXIDATION" in r["closure_gate_ids"]:
        audits=[x for x in r["source_audit_states"] if x["gate_id"]=="CHEM-REDOX-OXIDATION"]
        if len(audits)!=1 or audits[0]["status"]!="SOURCE_HARDENED": fail("CHEM_PASS_RECEIPT_INCONSISTENT","Redox closure requires exactly one SOURCE_HARDENED audit state")
...
def render(p):
    ...
    lines += ["","## Source authority hardening",""]
    if p["source_audit_states"]:
        for x in p["source_audit_states"]: lines.append(f"- `{x['gate_id']}` — {x['status']} — {x['scope_policy']} — `{x['audit_digest']}`")
    else: lines.append("- None required for this closure.")
