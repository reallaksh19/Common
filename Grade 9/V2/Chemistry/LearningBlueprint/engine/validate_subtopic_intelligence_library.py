#!/usr/bin/env python3
"""
Chemistry V2 — Subtopic Intelligence Library (SIL) Automated Validator
======================================================================
...
ATOM_TYPE_PATTERN = re.compile(r"[`]?ATOM-[A-Z0-9]+-\d+[`]?\s*\([`'\"']?(CONCEPT|RELATION|INVARIANT|PROCEDURE|STRATEGY)[`'\"']?\)")
MISCONCEPTION_PATTERN = re.compile(r"Misconception:", re.IGNORECASE)
TTU_PATTERN = re.compile(r"TTU-[A-Z0-9]+-\d+:", re.IGNORECASE)
FAMILY_PATTERN = re.compile(r"[`]?FAMILY-[A-Z0-9]+-\d+[`]?\s*\(", re.IGNORECASE)
PACKET_HEADER_PATTERN = re.compile(
    r"^##\s+(\d+)\.\s+(?:Concrete Exemplar|Foundation) Packet:\s+(.*?)\s+\(`(CHEM-[A-Z0-9-]+)`\)",
    re.MULTILINE,
)
...
def extract_packets(spec_text: str) -> List[Dict[str, Any]]:
    ...
    for i, match in enumerate(matches):
        ...
        # Truncate at Section Checklist if at the end
...
def validate_packet(packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    layer1_present = "Layer 1: Chemical Core" in body
    ...
    incomplete_scaffolds = len(re.findall(r"\[INCOMPLETE STATE", body, re.IGNORECASE))
    completion_keys = len(re.findall(r"\[COMPLETION KEY", body, re.IGNORECASE))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate Subtopic Intelligence Library packets for Chemistry")
    ...
    if args.json:
        ...
        print(f"Chemistry Subtopic Intelligence Library Validation: {status_badge}")
