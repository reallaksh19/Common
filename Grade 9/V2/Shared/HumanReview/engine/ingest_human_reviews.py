...
DIM_ORDER=('SUBJECT','PEDAGOGY','ASSESSMENT','VISUAL')
DIM_CLASS={'SUBJECT':'HUMAN_SUBJECT','PEDAGOGY':'HUMAN_PEDAGOGY','ASSESSMENT':'HUMAN_ASSESSMENT','VISUAL':'HUMAN_VISUAL'}
DEFAULT_DIM_GATE={'SUBJECT':'SUBJECT_CORRECTNESS','PEDAGOGY':'PEDAGOGY_USABILITY','ASSESSMENT':'ASSESSMENT_DESIGN','VISUAL':'VISUAL_USABILITY'}
LEGACY_DIMS=('SUBJECT','PEDAGOGY','VISUAL')
...
def active_dimensions(policy):
    dims=tuple(d for d in DIM_ORDER if d in policy['required_pass_submissions'])
    if set(dims)!=set(policy['required_pass_submissions']): raise ValueError('unknown review dimension in policy')
    return dims

def quality_gate_map(policy,dims):
    custom=policy.get('quality_state_by_dimension')
    if custom is None: return {d:DEFAULT_DIM_GATE[d] for d in dims}
    if set(custom)!=set(dims): raise ValueError('quality_state_by_dimension must cover active dimensions exactly')
    if len(set(custom.values()))!=len(custom): raise ValueError('quality state names must be unique')
    return dict(custom)

def candidate_artifact_refs(candidate):
    refs=candidate.get('artifact_sha256_refs') or [candidate['artifact_sha256']]
    if len(refs)!=len(set(refs)): raise ValueError('candidate artifact refs must be unique')
    return sorted(refs)

def load_rubrics(rubric_dir,required_dimensions=None):
    rubrics={}
    ...
    expected=set(required_dimensions or LEGACY_DIMS)
    if set(rubrics)!=expected: raise ValueError('rubrics must cover configured dimensions exactly once')
    return rubrics
...
def assess_submission(sub,candidate,registry,rubrics,mode,policy):
    ...
    dim=sub['review_dimension']; expected_class=DIM_CLASS.get(dim)
    if dim not in rubrics or expected_class is None: reasons.append('REVIEW_DIMENSION_NOT_CONFIGURED')
    if sub['candidate_artifact_sha256']!=candidate['artifact_sha256']: reasons.append('CANDIDATE_SHA_MISMATCH')
    expected_refs=candidate_artifact_refs(candidate)
    submitted_refs=sorted(sub.get('candidate_artifact_sha256_refs') or [])
    if policy.get('require_exact_artifact_set_binding') and not candidate.get('artifact_sha256_refs'):
        reasons.append('CANDIDATE_ARTIFACT_SET_NOT_CONFIGURED')
    if candidate.get('artifact_sha256_refs') and submitted_refs!=expected_refs:
    ...
    if expected_class not in registry['authorized_reviewers']:
        reasons.append('REVIEWER_CLASS_NOT_CONFIGURED')
    elif sub['reviewer_id'] not in registry['authorized_reviewers'][expected_class]:
        reasons.append('REVIEWER_NOT_AUTHORIZED_FOR_CLASS')
...
def project(candidate,registry,policy,rubrics,submissions,mode):
    ...
    dims=active_dimensions(policy); gates_by_dim=quality_gate_map(policy,dims)
    if set(rubrics)!=set(dims): raise ValueError('rubrics do not match policy dimensions')
    for dim in dims:
        if DIM_CLASS[dim] not in registry['authorized_reviewers']: raise ValueError('authorization registry missing '+DIM_CLASS[dim])
    ...
    accepted=[]; rejected=[]; accepted_by={d:[] for d in dims}
    for sub in submissions:
        sid,reasons=assess_submission(sub,candidate,registry,rubrics,mode,policy)
...
def main():
    ...
    a=ap.parse_args(); policy=load(a.policy); dims=active_dimensions(policy); result=project(load(a.candidate),load(a.registry),policy,load_rubrics(a.rubric_dir,dims),load_submissions(a.submissions_dir),a.mode); text=json.dumps(result,indent=2,sort_keys=True)+'\n'
