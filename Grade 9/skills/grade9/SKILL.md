...
- Physics subtopic-by-subtopic exact two-file build—Core Study Guide with Appendices A–C plus ExamSIDE Solution Book with concept/difficulty badges, H1–H3 and transfer diagnosis -> `../grade9-physics-subtopic-book-builder/SKILL.md`.
- Physics typed B30/B80/B90 pair schema, renderer and anti-drift audit -> `../grade9-physics-publication/SKILL.md`, with `../grade9-physics-examside/SKILL.md` for the external ExamSIDE ledger/reconciliation. These are executable adapters over `grade9-physics`/`grade9-physics-subtopic-book-builder`/`grade9-publication`; use `study_guide` + `transfer_book` with reciprocal pair identity for production. Legacy `core`/`question_bank` profiles are bounded demonstrations and do not satisfy a per-topic pair.
...
- Final chapter/whole-topic closeout after every subtopic is individually drafted and audited: full frozen-corpus backfill, one canonical primary home per item, chapter-level counters -> `../grade9-chapter-closeout-auditor/SKILL.md` (subject-agnostic; `grade9-redox-chapter-closeout-auditor` is its Chemistry/Redox profile).
