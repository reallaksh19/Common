...
def wrongnumber(x):x['questions'][0]['numeric_check']['distance']=999
def brokenrepair(x):x['questions'][0]['repair_target']='missing'
def nographtask(x):x['questions'][5]['figure']=None
def mismatch(x):x['questions'][5]['figure']['segments'][0][1]=3
def duplicate(x):x['questions'][1]['id']=x['questions'][0]['id']
def repeatedhint(x):x['questions'][0]['hints'][1]=x['questions'][0]['hints'][0]
def denominator(x):x['questions'].pop()
def unknown(x):x['unrendered_hidden_field']='must fail'
def missingregion(x):x['questions'][5]['figure']['segments'].pop()
def missingcitation(x):x['questions'][0]['source_status']='SOURCE_VERIFIED'
...
v=copy.deepcopy(d);q=v['questions'][0];q['source_status']='ADAPTED'
