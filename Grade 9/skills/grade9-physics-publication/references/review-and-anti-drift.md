# Anti-drift and approval

Freeze source role, allowed rewrites, band profiles, same-topic requirement and approval boundary. Back up previous PDFs/models/skills. Pin repository sources and keep attachments unchanged. For source-preserving rebuilds, freeze every source obligation; retain PRESERVED/RECOMPOSED/MERGED/SPLIT/INTENTIONALLY_ABSENT/REVIEW_REQUIRED states. For an authorised rewrite, document retained, rewritten, added and out-of-scope items without a zero-loss claim.

## Blocking failures

| Gate | Reject |
|---|---|
| Source/scope | Invented syllabus/exam/source-complete claims or silent replacements |
| Physics | Undefined direction, wrong graph type, invalid shape formula, wrong unit/answer, absent model condition |
...
| Links | Missing destination, wrong pairing, misleading external citation |
...
After every completed subtopic/set, repair blockers before scaling. After revisions re-run affected checks and freeze the final bytes. Present Physics for approval before advancing to Mathematics/Chemistry when the user asks for that order.
