...
from v3b.contracts import digest, file_digest, load, verify_file
...
def publish(plan_path: Path, baseline_path: Path, source_root: Path, out: Path):
    plan, baseline = load(plan_path), load(baseline_path)
    ctx = read_inputs(plan, baseline, source_root)
    ...
    def write(staging):
        ...
        manifest = {"basis_digest": basis, "runtime_digest": digest(runtime_manifest()),
        ...
        return verify_publication(staging, basis)
