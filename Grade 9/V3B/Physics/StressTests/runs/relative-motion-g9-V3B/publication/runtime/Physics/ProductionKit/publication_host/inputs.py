...
from v3b.contracts import digest, load, require, strings, text, unique, validate_dag, verify_file

CORES = {"CORE1A", "CORE1B", "CORE2A", "CORE2B"}
KINDS = {"TEXT", "EQUATION", "QUESTION", "FIGURE"}


def read_inputs(plan: dict, baseline: dict, source_root: Path) -> dict:
    require(plan.get("schema_version") == baseline.get("schema_version") == "1.0.0",
            "PUBLICATION_SCHEMA_UNSUPPORTED")
    require(plan.get("subject") == "Physics", "WRONG_SUBJECT_KIT")
    ...
    context = dict(plan=plan, baseline=baseline, sources=sources, atoms=atoms,
                   questions=questions, buckets=buckets, obligations=obligations)
