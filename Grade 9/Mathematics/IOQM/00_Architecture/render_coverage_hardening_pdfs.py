...
def _texify(s: str) -> str:
    ...
    t = t.replace("*", r"\cdot ")
    # Repository prose often spells congruence to remain transport-safe.
    t = re.sub(r"\bcongruent\b", r"\\equiv", t)
    return t


def _looks_math(s: str) -> bool:
    if "/" in s or re.search(r"\.(md|csv|py|pdf)$", s, re.I):
        return False
    if re.fullmatch(r"[A-Z][A-Z0-9 _\-]+", s):
        return False
    if re.search(r"[A-Za-z]{4,}\s+[A-Za-z]{4,}", s) and not re.search(r"[=<>≡∤φ²³·^]", s):
        return False
    return bool(
        re.search(
            r"(sqrt|\^|_[A-Za-z0-9(]|[=<>]|≡|∤|φ|²|³|·|\bmod\b|\bgcd\b|\blcm\b|\bphi\b|\bcongruent\b|\d+\s*[+\-*/]\s*\d+|[A-Za-z]\|[A-Za-z0-9(])",
            s,
        )
    )
...
def _cover(title: str, subtitle: str, audience: str, footer: str, router: str) -> str:
    safe_title = title.replace("&", r"\&")
    safe_sub = subtitle.replace("&", r"\&")
    safe_aud = audience.replace("&", r"\&")
    safe_footer = footer.replace("&", r"\&")
    safe_router = router.replace("->", r"$\rightarrow$").replace("&", r"\&")
    return rf'''```{{=latex}}
\thispagestyle{{empty}}
\vspace*{{22mm}}
{{\sffamily\bfseries\small IOQM · GRADE 9 MATHEMATICS}}\\[16mm]
{{\sffamily\bfseries\fontsize{{25}}{{29}}\selectfont {safe_title}}}\\[7mm]
{{\sffamily\fontsize{{13}}{{17}}\selectfont\color{{darkgray}} {safe_sub}}}\\[10mm]
\rule{{38mm}}{{1.1pt}}\\[8mm]
{{\ttfamily\small {safe_router}}}\\[20mm]
{{\sffamily\small\color{{darkgray}} {safe_aud}}}
\vfill
{{\sffamily\footnotesize {safe_footer}}}
\newpage
```
'''


def _combine(root: Path, files, title, subtitle, audience, footer, router, breaks=True):
...
def _render(md_text: str, output: Path, header_left: str, header_right: str):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        header_path.write_text(
            HEADER.replace("HEADER_LEFT", header_left).replace("HEADER_RIGHT", header_right),
            encoding="utf-8",
        )
        cmd = [
...
def render_topic(code: str):
    cfg = CONFIG[code]
    root = cfg["root"]
    pdf_dir = root / "PDFs"
    student = _combine(
        root, cfg["student_files"], cfg["title"], cfg["subtitle"],
        "Student learning pack - assimilation, recognition, transfer and mastery",
        "Grade 9 IOQM learning pack", cfg["router"], breaks=True,
    )
    teacher = _combine(
        root, cfg["teacher_files"], cfg["title"], "Diagnostic and coverage-hardening companion",
        "Teacher-only companion - answer diagnostics and theorem-hypothesis checks",
        "Grade 9 IOQM teacher companion", cfg["router"], breaks=False,
    )
