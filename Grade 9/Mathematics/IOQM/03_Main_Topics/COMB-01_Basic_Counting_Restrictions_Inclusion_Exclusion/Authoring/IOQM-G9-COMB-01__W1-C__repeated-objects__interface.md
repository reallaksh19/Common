...
microstream_id: W1-C
microstream_title: Repeated objects and identity
...
## A. Scope boundary
Included: multiset arrangements and indistinguishable copies. Excluded: generic recurrence/state evolution, graph-coloring canon and arithmetic digit-rule derivation.
...
divide only by permutations inside classes whose swaps do not change the counted object. The topic router is `DEFINE OBJECT -> IDENTITY/ORDER -> RESTRICTIONS -> CASES/STAGES -> DIRECT/COMPLEMENT/IE -> COUNT -> CHECK`.
...
IOQM-2025-Q18. Exact source/key custody and independent routes are recorded in the source map and audit.
