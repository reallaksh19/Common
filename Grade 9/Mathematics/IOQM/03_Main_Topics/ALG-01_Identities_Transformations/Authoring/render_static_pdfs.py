...
def build(files, out, label, fontsize=7.2, leading=8.7, lines_per_page=80):
    pages, current = [], []
    ...
    for page_number, lines in enumerate(pages, 1):
        ...
        objects.append(
            f"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents {content_obj} 0 R >>".encode()
        )
        ...
        for line_number, line in enumerate(lines):
            ...
            commands.append("T*")
        commands += [
            "ET", "BT", "/F1 6 Tf", f"30 18 Td ({label}) Tj",
            f"500 0 Td (Page {page_number}/{len(pages)}) Tj", "ET",
        ]
        compressed = zlib.compress("\n".join(commands).encode("ascii"), 9)
        encoded = base64.a85encode(compressed, adobe=False, wrapcol=0)
        objects.append(
            b"<< /Filter [/ASCII85Decode /FlateDecode] /Length " + str(len(encoded) + 2).encode()
            + b" >>\nstream\n" + encoded + b"~>\nendstream"
        )
...
build(STUDENT, PDFS / "ALG01_Student_Pack_v1.pdf", "ALG-01 Student Pack")
build(["Teacher_Diagnostic_Key.md"], PDFS / "ALG01_Teacher_Key_v1.pdf", "ALG-01 Teacher Key", 7.0, 8.4, 84)
