# ALG-01 - Practice and Transfer Bank

Attempt every item before consulting support or the teacher key.

## F0 - reconnect

1. Factor `x^2-49`.
2. Expand `(2x-3)^2`.
3. If `a+b=11` and `ab=24`, find `a^2+b^2`.
4. Evaluate `52^2-48^2` structurally.
5. State whether `(x+1)^2=x^2+2x+1` is an identity or a solution-only relation.

## F1 - direct representation choice

6. Evaluate `1004^2-996^2` without long multiplication.
7. If `x^2=5x-3`, reduce `x^4` to `Ax+B`.
8. Set a substitution to reduce `(y+1/y)^2-7(y+1/y)+10=0`, `y!=0`.
9. Given `u+v=8`, `uv=12`, find `u^3+v^3`.
10. Solve `(x-7)(x+2)=0` using the form already given.
...
16. A number `x` satisfies `x^2-x-1=0`. Compute `x^8` in the form `Ax+B`.
...
21. Given `q^2=2q+1`, compute `q^7-13q^3` as `Aq+B`.
22. If `a+b=6` and `ab=5`, determine `(a-b)^4` without finding `a,b`.
23. Let `w+1/w=2`, `w!=0`. Find `w^8+1/w^8` using structure, not individual root solving.
