...
microstream_id: W2-G
microstream_title: Common-root elimination
...
## A. Scope boundary
Included: subtract or combine polynomial equations to lower degree, then verify candidates in all originals. Excluded: canonical inequality/equality/attainment teaching and prerequisite transformation canon.
...
a common root satisfies every polynomial combination, but an eliminated candidate must be checked in each original equation. The governing principle is `REQUESTED INFORMATION -> CHEAPEST VALID REPRESENTATION -> CALCULATION -> CHECK`.
...
IOQM-2023-Q12. Exact paper/key custody and independent reconstruction are recorded in the source coverage and mathematics audit.
