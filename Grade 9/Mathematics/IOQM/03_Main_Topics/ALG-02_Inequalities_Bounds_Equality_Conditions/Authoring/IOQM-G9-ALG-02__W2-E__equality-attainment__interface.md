...
microstream_id: W2-E
microstream_title: Equality conditions and attainment
...
## A. Scope boundary
Included: separate numerical bound, equality condition and admissibility in the actual domain. Excluded: Vieta/discriminant/root-feasibility canon, which remains with the polynomial owner, and prerequisite algebraic transformation teaching.
...
equality outside the domain blocks minimum/maximum claims. Every extremum claim must pass `BOUND -> EQUALITY -> ATTAINMENT`, followed by a discrete filter when the domain requires it.
