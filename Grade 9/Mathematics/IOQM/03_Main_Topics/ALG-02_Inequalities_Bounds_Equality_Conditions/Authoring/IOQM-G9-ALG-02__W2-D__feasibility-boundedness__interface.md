...
microstream_id: W2-D
microstream_title: Feasibility, boundedness and direction
...
## A. Scope boundary
Included: identify requested direction and whether a finite bound exists before choosing a technique. Excluded: Vieta/discriminant/root-feasibility canon, which remains with the polynomial owner, and prerequisite algebraic transformation teaching.
...
a true bound in the wrong direction or an unattained infimum/supremum is not the requested extremum. Every extremum claim must pass `BOUND -> EQUALITY -> ATTAINMENT`, followed by a discrete filter when the domain requires it.
