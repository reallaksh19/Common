...
microstream_id: W2-B
microstream_title: Cauchy and Engel representation
...
## A. Scope boundary
Included: justified reciprocal/sum bounds using Cauchy/Engel only when the structure exposes the requested direction. Excluded: Vieta/discriminant/root-feasibility canon, which remains with the polynomial owner, and prerequisite algebraic transformation teaching.
...
the theorem name is secondary; equality and domain hypotheses must be checked. Every extremum claim must pass `BOUND -> EQUALITY -> ATTAINMENT`, followed by a discrete filter when the domain requires it.
...
IOQM-2024-Q06. Exact source custody and independent reconstruction are recorded in the source map/audit.
