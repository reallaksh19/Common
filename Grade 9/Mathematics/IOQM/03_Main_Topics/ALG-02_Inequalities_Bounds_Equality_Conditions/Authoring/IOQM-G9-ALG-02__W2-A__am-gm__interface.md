...
microstream_id: W2-A
microstream_title: AM-GM and balance bounds
...
## A. Scope boundary
Included: positive-term AM-GM, fixed-product lower bounds and fixed-sum product upper bounds. Excluded: Vieta/discriminant/root-feasibility canon, which remains with the polynomial owner, and prerequisite algebraic transformation teaching.
...
equality occurs at the balanced configuration under the stated positive/nonnegative domain. Every extremum claim must pass `BOUND -> EQUALITY -> ATTAINMENT`, followed by a discrete filter when the domain requires it.
