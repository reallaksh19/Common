...
microstream_id: W2-F
microstream_title: Real-to-discrete filtering
...
## A. Scope boundary
Included: use a continuous bound to localize candidates, then check the admissible integers/discrete set. Excluded: Vieta/discriminant/root-feasibility canon, which remains with the polynomial owner, and prerequisite algebraic transformation teaching.
...
the real equality point cannot be imported directly into an integer domain. Every extremum claim must pass `BOUND -> EQUALITY -> ATTAINMENT`, followed by a discrete filter when the domain requires it.
