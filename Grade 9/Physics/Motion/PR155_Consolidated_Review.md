...
Everything from the original 20 review comments (A-O) is closed. Not closed, and out of scope for this PR:

- **Full chapter-scale Motion build.** `grade9-chapter-closeout-auditor` (finding G) and the master-schema conformance (§6) give the infrastructure; actually running the 68-question Motion chapter through `grade9-physics-subtopic-book-builder` + this closeout auditor is a separate, much larger content-production task.
- **SRU acceptance data.** The schema (finding K) is real and machine-checked; the actual pedagogical judgment calls for B30/B80's 4 concepts × 15 dimensions are deliberately left to an independent reviewer, not pre-filled here.
- **Hint-numbering convention unification** (the aside after §5's table) across `grade9-learning-enrichment`/`grade9-physics-subtopic-book-builder`/`grade9-math-assimilation`.
- **Figure rendering genericity** beyond the classification layer — `render_v2.py`'s `vt` renderer is still hardcoded to velocity/time axis labels, not yet a parametric 2-axis grapher reusable for other subjects' graphs.
