...
class CoreTests(unittest.TestCase):
    def test_good_repo(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for check in (repo_state,repo_profile,roadmap,frontier,ep_check,ep_semantics,acceptance,progress_check,serial,state_planes,projection,drift,checkpoint_linkage,takeover,baton_readiness,cold_start):self.assertEqual([],check(root)[0],check.__module__)
    ...
    def test_terminal_repo_has_empty_frontier_and_no_active_ep(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual([],frontier(root)[0]);self.assertEqual([],projection(root)[0]);self.assertEqual([],checkpoint_linkage(root)[0]);self.assertEqual([],baton_readiness(root)[0]);self.assertEqual([],cold_start(root)[0])
