...
def good(root):
    ...
    s={"schema_version":"relay-v2.5","repository":{"name":"test"},"roadmap":{"id":"RM-T","revision":"RM-0001","path":"agents/relay/roadmap/OVERALL_ROADMAP.yaml"},"current_position":{"objective":"OBJ-1","phase":"PHASE-1","work_package":"WP-1"},"execution_policy":{"mode":"SERIAL"},"active_ep":{"id":"EP-1","path":"agents/relay/execution-packages/EP-1.yaml","state":"ACTIVE"},"progress":{"overall_percent":50,"phase_percent":50,"ep_percent":50,"basis_revision":"PB-1"},"status_planes":{"execution":{"state":"ACTIVE","can_continue":True,"next_action":"Implement AC-1"},"quality":{"state":"CLEAR","findings":[]},"evidence":{"state":"PARTIAL","summary":"One required check remains","not_run":[]},"stop":{"active":False,"category":"NONE","reason":"","basis":[]}},"chat_context_required":False}
    dump(root/"agents/relay/roadmap/OVERALL_ROADMAP.yaml",r);dump(root/"agents/relay/roadmap/PROGRESS.yaml",p);dump(root/"agents/relay/execution-packages/EP-1.yaml",ep);dump(root/"agents/relay/REPO_STATE.yaml",s);return r,ep,p,s

class CoreTests(unittest.TestCase):
    def test_good_repo(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);good(root)
            for check in (repo_state,roadmap,frontier,ep_check,acceptance,progress_check,serial,state_planes,cold_start):self.assertEqual([],check(root)[0],check.__module__)
