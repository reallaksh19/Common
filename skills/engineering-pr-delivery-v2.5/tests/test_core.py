...
def good(root):
    ...
    ep={"schema_version":"relay-v2.5","identity":{"ep_id":"EP-1","branch":"agent/test","base_ref":"abc","execution_state":"ACTIVE","previous_checkpoint":"NONE"},"git_basis":{"expected_branch":"agent/test","material_ref":"abc","base_branch":"main","base_observed_ref":"base-1","drift_policy":"RECHECK_BEFORE_WRITE","drift_receipt":None},"roadmap_source":{"roadmap_id":"RM-T","roadmap_revision":"RM-0001","objective":"OBJ-1","phase":"PHASE-1","work_package":"WP-1","generated_from_frontier":True},"outcome":{"user_visible":["User sees result"],"engineering":["Invariant preserved"]},"context_capsule":{"product_goal":"Deliver feature","roadmap_position":"Phase 1","why_this_work_exists":"Required by roadmap","current_architecture":"Single service","prior_owner_decisions":[],"current_implementation_state":"Baseline exists","known_problems":[],"deliberate_non_goals":[],"terminology":{}},"repository_discovery":[{"id":"DISC-1","action":"VERIFY","target":"agents/relay/REPO_STATE.yaml","purpose":"confirm"}],"inputs":[{"id":"INPUT-1","name":"policy","source":"roadmap","editable":False}],"benchmarks":[],"scope":{"allowed":[{"path":"src/a.py","reason":"implementation"}],"prohibited":[{"domain":"solver","reason":"outside EP"}]},"anti_drift":{"do_not":["change solver"],"stale_if":["roadmap changes"]},"implementation_plan":[{"id":"STEP-1","action":"Implement AC-1"}],"quality":{"blueprints":["coding","testing"]},"acceptance":[{"id":"AC-1","description":"Works","weight":100,"verification":["TEST-1"]}],"validation":[{"id":"TEST-1","class":"MUST_PASS","method":"pytest","proves":["AC-1"]}],"failure_and_stop_conditions":{"hard_stop_categories":["ROADMAP_CONFLICT"]},"report_contract":{"sections":REPORT_SECTIONS},"checkpoint_contract":{"required":True},"successor_relay":{"required":True,"duties":["compute next executable frontier","run cold-start check","update REPO_STATE"]}}
    ...
    s={"schema_version":"relay-v2.5","relay_state":"ACTIVE","repository":{"name":"test"},"roadmap":{"id":"RM-T","revision":"RM-0001","path":"agents/relay/roadmap/OVERALL_ROADMAP.yaml"},"current_position":{"objective":"OBJ-1","phase":"PHASE-1","work_package":"WP-1","work_packages":[]},"execution_policy":{"mode":"SERIAL"},"active_ep":{"id":"EP-1","path":"agents/relay/execution-packages/EP-1.yaml","state":"ACTIVE"},"last_checkpoint":{"id":"NONE","path":None},"progress":{"overall_percent":50,"phase_percent":50,"ep_percent":50,"basis_revision":"PB-1"},"status_planes":{"execution":{"state":"ACTIVE","can_continue":True,"next_action":"Implement AC-1"},"quality":{"state":"CLEAR","findings":[]},"evidence":{"state":"PARTIAL","summary":"One required check remains","not_run":[]},"stop":{"active":False,"category":"NONE","reason":"","basis":[]}},"projection":{"required":False,"state":"NOT_REQUIRED","roadmap_revision":"RM-0001","execution_ref":"EP-1","basis":[]},"relay_readiness":{"repository_ready":True,"projection_ready":True,"handover_ready":True,"reasons":[]},"chat_context_required":False}
...
class CoreTests(unittest.TestCase):
    def test_good_repo(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for check in (repo_state,roadmap,frontier,ep_check,acceptance,progress_check,serial,state_planes,projection,drift,checkpoint_linkage,cold_start):self.assertEqual([],check(root)[0],check.__module__)
