...
def lane_quality_review(root:Path,ep_id:str):
    ep_path=root/f"agents/relay/execution-packages/{ep_id}.yaml";ep_data=yaml.safe_load(ep_path.read_text());qid=f"QRV-{ep_id}";qpath=root/f"agents/relay/quality/{qid}.yaml"
    qrv={"schema_version":"relay-v2.5-quality-review","quality_review_id":qid,"ep":{"id":ep_id,"path":str(ep_path.relative_to(root)),"contract_digest":yaml_digest(ep_path)},"roadmap_revision":"RM-1","material_ref":"abc","reviewer":{"type":"DETERMINISTIC_VALIDATOR","identity":f"synthetic-quality-review-{ep_id}"},"router_snapshot":router_snapshot(ep_data),"procedure_results":[{"blueprint":"coding","result":"CLEAR","evidence":[f"{ep_id} code review evidence"]},{"blueprint":"testing","result":"CLEAR","evidence":[f"{ep_id} test review evidence"]}],"findings":[],"overall_state":"CLEAR","execution_effect":{"blocks_execution":False,"blocking_findings":[]},"owner_report":{"summary":f"Applicable quality procedures are clear for {ep_id}.","visible_risks":[],"decisions_required":[]},"successor_handover":{"unresolved_findings":[],"follow_up":[]}}
    dump(qpath,qrv);return {"id":qid,"path":str(qpath.relative_to(root)),"digest":yaml_digest(qpath)}
def lane_cp(cp_id,ep_id,lane_id,quality_review,plan_id="PLAN-P1"):
    return {"schema_version":"relay-v2.5","checkpoint_id":cp_id,"ep_id":ep_id,"roadmap_basis":{"roadmap_id":"RM-P","revision":"RM-1"},"execution_basis":{"material_ref":"abc"},"implementation_result":{"completed_steps":["STEP-1"],"files_changed":[]},"acceptance_results":[],"validation_results":[],"quality_review":quality_review,"quality_findings":[],"discoveries":[],"roadmap_reconciliation":{"result":"STATUS_UPDATE"},"successor":{"mode":"JOIN","frontier_work_package":None,"ep_id":None,"parallel_plan":plan_id,"lane_id":lane_id,"lanes":[]}}
