...
def converge(root:Path):
    ...
    phase=roadmap["objectives"][0]["phases"][0];a,b,integrate=phase["work_packages"]
    a["state"]="COMPLETE";a["execution_status"]="TERMINAL";b["state"]="COMPLETE";b["execution_status"]="TERMINAL";integrate["state"]="ACTIVE";integrate["execution_status"]="ACTIVE"
    integration_ep=ep("EP-I","agent/integration","WP-I","src/integration");integration_ep["identity"]["previous_join"]="JOIN-P1"
    ...
    state["relay_state"]="ACTIVE";state["current_position"]={"objective":"OBJ","phase":"PHASE","work_package":"WP-I","work_packages":[]};state["execution_policy"]={"mode":"SERIAL"};state["active_ep"]={"id":"EP-I","path":"agents/relay/execution-packages/EP-I.yaml","state":"ACTIVE"};state["last_checkpoint"]={"id":"NONE","path":None};state["predecessor_join"]={"id":"JOIN-P1","path":"agents/relay/parallel/JOIN-P1.yaml"};state["status_planes"]["execution"]={"state":"ACTIVE","can_continue":True,"material_authority":"WRITE","next_action":"Execute the integration EP after all lane checkpoints converge."};state["projection"].update({"execution_ref":"EP-I"})
    dump(root/"agents/relay/roadmap/OVERALL_ROADMAP.yaml",roadmap);dump(root/"agents/relay/REPO_STATE.yaml",state);dump(root/"agents/relay/execution-packages/EP-I.yaml",integration_ep);dump(root/"agents/relay/checkpoints/CP-A.yaml",lane_cp("CP-A","EP-A","LANE-A"));dump(root/"agents/relay/checkpoints/CP-B.yaml",lane_cp("CP-B","EP-B","LANE-B"));dump(root/"agents/relay/parallel/JOIN-P1.yaml",join)
    return roadmap,plan,state,join
...
class ParallelConvergenceStressTests(unittest.TestCase):
    def test_parallel_join_becomes_cold_startable_serial_integration(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual([],parallel_join(root)[0]);self.assertEqual([],checkpoint_linkage(root)[0]);self.assertEqual([],cold_start(root)[0])

    def test_join_fails_if_one_lane_checkpoint_is_missing(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);converge(root);(root/"agents/relay/checkpoints/CP-B.yaml").unlink()
            self.assertTrue(any("lane checkpoint missing" in x for x in parallel_join(root)[0]))

    def test_join_fails_before_all_lane_work_packages_are_complete(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);roadmap,_,_,_=converge(root);roadmap["objectives"][0]["phases"][0]["work_packages"][1]["state"]="ACTIVE";roadmap["objectives"][0]["phases"][0]["work_packages"][1]["execution_status"]="ACTIVE";dump(root/"agents/relay/roadmap/OVERALL_ROADMAP.yaml",roadmap)
            errors=parallel_join(root)[0];self.assertTrue(any("sole computed frontier" in x or "requires lane work package COMPLETE" in x for x in errors))

    def test_join_fails_if_integration_ep_does_not_bind_join(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);converge(root);path=root/"agents/relay/execution-packages/EP-I.yaml";data=yaml.safe_load(path.read_text());data["identity"]["previous_join"]="JOIN-WRONG";dump(path,data)
            self.assertTrue(any("previous_join" in x for x in parallel_join(root)[0]))
