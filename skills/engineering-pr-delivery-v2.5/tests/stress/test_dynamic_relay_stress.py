...
def repo_state(evidence_state="PARTIAL",not_run=None,execution_state="ACTIVE",stop=None):
    return {"schema_version":"relay-v2.5","status_planes":{"execution":{"state":execution_state,"can_continue":True,"next_action":"Execute the current bounded validation step."},"quality":{"state":"CLEAR","findings":[]},"evidence":{"state":evidence_state,"summary":"Synthetic evidence state","not_run":not_run or []},"stop":stop or {"active":False,"category":"NONE","reason":"","basis":[]}}}

class DynamicRelayStressTests(unittest.TestCase):
    def test_infrastructure_not_run_is_evidence_not_hard_stop(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);dump(root/"agents/relay/REPO_STATE.yaml",repo_state("NOT_RUN",[{"id":"TEST-EXT","reason":"External runner created no executable job","cause":"INFRASTRUCTURE"}]))
