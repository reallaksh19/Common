...
class RendererStressTests(unittest.TestCase):
    def test_parallel_renderers_show_lanes_and_integration(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);parallel_repo(root)
            status=render_status(root);handover=render_handover(root)
            self.assertIn("Parallel plan",status);self.assertIn("LANE-A",status);self.assertIn("Integration",status)
            self.assertIn("Parallel plan PLAN-P1",handover);self.assertIn("EP-A",handover);self.assertIn("WP-I",handover)

    def test_initializing_renderers_do_not_require_an_ep(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            status=render_status(root);handover=render_handover(root)
            self.assertIn("Initializing",status);self.assertIn("NONE",status)
            self.assertIn("Initializing",handover);self.assertIn("No active material EP",handover)
