...
from qualitylib import BLUEPRINTS,current_eps,text
...
def validate_ep(ep:dict,label:str):
    ...
    for key in ("applicable","not_applicable","review_contract","findings_policy","ordinary_findings_are_hard_stops"):
        if key not in q:e.append(f"{label}.quality missing {key}")
    seen={}
    for bucket in ("applicable","not_applicable"):
        vals=q.get(bucket)
        ...
        for i,item in enumerate(vals):
            ...
            elif focus not in {None} and (not isinstance(focus,list) or not all(text(x) for x in focus)):
                e.append(f"{il}.review_focus, when present, must contain explicit text")
