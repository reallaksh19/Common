...
def validate(root:Path):
    ...
    relay_state=s.get("relay_state");active=s.get("active_ep") or {};checkpoint_id=last.get("id");checkpoint_path=last.get("path")
    ...
    if relay_state=="ACTIVE":
        if mode!="SERIAL":e.append("ACTIVE relay requires last checkpoint successor.mode SERIAL")
        ep=load_yaml(root/active["path"]);previous=(ep.get("identity") or {}).get("previous_checkpoint")
