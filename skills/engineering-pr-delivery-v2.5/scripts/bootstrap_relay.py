...
def _required_manifest(manifest:dict)->list[str]:
    ...
    e+=require(manifest.get("repository") or {},["name","remote","repository_type","default_branch"],"BOOTSTRAP_MANIFEST.repository")
    e+=require(manifest.get("relay_protocol") or {},["basis_ref"],"BOOTSTRAP_MANIFEST.relay_protocol")
    e+=require(manifest.get("roadmap") or {},["id","revision","title"],"BOOTSTRAP_MANIFEST.roadmap")
...
def build(manifest:dict)->dict[str,dict]:
    ...
    repo=manifest["repository"];proto=manifest["relay_protocol"];road=manifest["roadmap"];pos=manifest["initial_position"];init=manifest["initialization"]
    oid=pos["objective"]["id"];pid=pos["phase"]["id"];wid=pos["work_package"]["id"]
    roadmap={
        "schema_version":"relay-v2.5",
        "roadmap":{"id":road["id"],"revision":road["revision"],"title":road["title"]},
        "objectives":[{
            "id":oid,"title":pos["objective"]["title"],"state":"ACTIVE","definition":"PARTIALLY_DEFINED",
            "phases":[{
                "id":pid,"title":pos["phase"]["title"],"state":"PLANNED","definition":"OWNER_REVIEW_REQUIRED",
                "work_packages":[{
                    "id":wid,"title":pos["work_package"]["title"],"state":"PLANNED","definition":"OWNER_REVIEW_REQUIRED","execution_status":"WAITING","depends_on":[]
                }]
            }]
        }]
    }
    state={
        "schema_version":"relay-v2.5","relay_state":"INITIALIZING",
        "repository":{"name":repo["name"],"remote":repo["remote"]},
        "relay_protocol":{"version":"2.5","basis_ref":proto["basis_ref"]},
        "roadmap":{"id":road["id"],"revision":road["revision"],"path":"agents/relay/roadmap/OVERALL_ROADMAP.yaml"},
        "current_position":{"objective":oid,"phase":pid,"work_package":wid,"work_packages":[]},
    ...
    profile={
        "schema_version":"relay-v2.5","repository_type":repo["repository_type"],"default_branch":repo["default_branch"],
        "important_paths":{},"commands":{},"protected_domains":[],"generated_paths":[],"repository_rules":[]
    }
    progress={
        "schema_version":"relay-v2.5","progress_basis":{"id":"PB-0001","roadmap_revision":road["revision"]},
        "overall":{"earned_weight":0,"total_weight":0,"percent":0},"objectives":[],"phases":[],"work_packages":[],"execution_packages":[]
    }
    issue_graph={"schema_version":"relay-v2.5","nodes":[],"relationships":[]}
    return {
        "agents/relay/REPO_STATE.yaml":state,
        "agents/relay/REPO_PROFILE.yaml":profile,
        "agents/relay/roadmap/OVERALL_ROADMAP.yaml":roadmap,
        "agents/relay/roadmap/PROGRESS.yaml":progress,
        "agents/relay/roadmap/ISSUE_GRAPH.yaml":issue_graph,
    }
...
def main():
    ap=argparse.ArgumentParser(description="Create a repository-neutral V2.5 INITIALIZING relay scaffold. Dry-run by default.")
    ap.add_argument("manifest");ap.add_argument("repo_root",nargs="?",default=".");ap.add_argument("--apply",action="store_true")
    a=ap.parse_args();manifest=load_yaml(Path(a.manifest).resolve());files=build(manifest);root=Path(a.repo_root).resolve()
    print("V2.5 bootstrap plan:")
    for path in files:print(f"  {'WRITE' if a.apply else 'WOULD WRITE'} {path}")
    if a.apply:
        apply(root,files);print("Bootstrap state: INITIALIZING. No EP was fabricated; reconcile roadmap and compute a frontier before material work.")
